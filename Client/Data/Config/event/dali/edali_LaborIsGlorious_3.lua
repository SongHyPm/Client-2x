--MisDescBegin
x210249_g_ScriptId 						= 210249
x210249_g_Position_X 					= 172.7304
x210249_g_Position_Z 					= 146.4640
x210249_g_SceneID 						= 2
x210249_g_AccomplishNPC_Name	= "Tôn Bát Gia"
x210249_g_MissionId 					= 1154
x210249_g_Name 								= "Tôn Bát Gia"
x210249_g_MissionKind 				= 13
x210249_g_MissionRound				= 288
x210249_g_MissionLevel 				= 10000
x210249_g_IfMissionElite 			= 0
x210249_g_IsMissionOkFail			= 0		
x210249_g_MissionName 				= "Lao ðµng t¯i quang vinh"
x210249_g_MissionInfo					= ""
x210249_g_MissionTarget				= "%f"
x210249_g_StrForePart 				= 4
x210249_g_FormatList = {"Mong các hÕ ðªn %s ð¬ trong hÕt gi¯ng hy v÷ng này, sau khi hoàn thành ðªn ÐÕi Lý Tôn Bát Gia lãnh nh§n ph¥n thß·ng nhi®m vø Lao ðµng là vinh quang.#r#G Chú ý: Sñ ki®n này m²i ngày chï có th¬ tham gia mµt l¥n, nªu nhß bö nhi®m vø ngày hôm nay thì không th¬ tiªp tøc tham gia sñ ki®n Lao ðµng là vinh quang trong ngày.",}
x210249_g_StrList = {
"Kiªm Các#{_INFOAIM58,189,7,}",
"NhÕn Nam#{_INFOAIM168,114,18,}",
"Vô Lßþng S½n#{_INFOAIM249,42,6,}",
"Thái H°#{_INFOAIM97,177,4,}",
"Tung S½n#{_INFOAIM132,198,3,}",
}
x210249_g_ControlScript				= 210249
x210249_g_Custom							= { {id="Ðã tr°ng hÕt gi¯ng",num=1}}
--MisDescEnd
