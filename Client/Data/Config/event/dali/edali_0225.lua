--MisDescBegin
x210225_g_ScriptId = 210225
x210225_g_Position_X=214.8831
x210225_g_Position_Z=283.8709
x210225_g_SceneID=2
x210225_g_AccomplishNPC_Name="Ðoàn Diên Khánh"
x210225_g_MissionId = 705
x210225_g_MissionIdPre = 704
x210225_g_Name	="Ðoàn Diên Khánh"
x210225_g_MissionKind = 13
x210225_g_MissionLevel = 9
x210225_g_IfMissionElite = 0
x210225_g_MissionName="Ðoàn Diên Khánh"
x210225_g_MissionInfo="#{event_dali_0035}"
x210225_g_MissionTarget="— #Ghëm Ng÷c Nhî phía Ðông Nam thành ÐÕi Lý#W tìm #RÐoàn Diên Khánh#W#{_INFOAIM215,284,2,Ðoàn Diên Khánh}. #b#G (Dùng chuµt trái nh¤p vào t÷a ðµ, có th¬ giúp các hÕ tìm ðßþc NPC) #l"
x210225_g_MissionComplete="#{event_dali_0036}"
x210225_g_MoneyBonus=72
x210225_g_SignPost = {x = 215, z = 284, tip = "Ðoàn Diên Khánh"}
x210225_g_Custom	= { {id="Ðã tìm ðßþc Ðoàn Diên Khánh",num=1} }
x210225_g_IsMissionOkFail = 1		--±äÁ¿µÄµÚ0Î»
--MisDescEnd
