...
local RuleTable = {
	{
		...
		maxGrade = 9,
		msgGradeLimited = "Bäo thÕch hşp thành c¤p cao nh¤t là c¤p 9, bäo thÕch cüa b¢ng hæu không th¬ tiªp tøc hşp thành næa.",
	...
	{
		msgDiffTypeErr = "B¡t buµc phäi sØ døng cùng mµt loÕi nguyên li®u m¾i hşp thành.",
		msgDiffGradeErr = "B¡t buµc nguyên li®u có ğÆng c¤p ngang nhau m¾i hşp thành.",
		msgLackMoney = "Không ğü #{_EXCHG%d} vàng. ",
		msgLackStuff = "M²i l¥n c¤t v§t ph¦m hşp thành phäi nhi«u h½n 2 chiªc.",
		msgSlotEmpty = "Hãy ğßa nguyên li®u c¥n hşp thành vào.",				-- add by zchw
		maxGrade = 8,	-- ÎïÆ·±íÀïµÄµÈ¼¶£¬3¼¶²ÄÁÏÎª4¼¶£¬ËùÒÔ±ß½çÎª5¼¶ mark by cuiyinjie maxGrade add 1
		msgGradeLimited = "Cao nh¤t có th¬ ğ¬ vào nguyên li®u c¤p 8, nguyên li®u cüa các hÕ không th¬ tiªp tøc hşp thành.", --µÈ¼¶¼Ó1¼¶
...
function MaterialCompound_OnEvent( event )
	if event == "UI_COMMAND" and tonumber( arg0 ) == 23 then	-- ±¦Ê¯ºÏ³É
		...
		MaterialCompose_Info : SetText("Hşp thành bäo thÕch có th¬ làm cho nhæng bäo thÕch s½ c¤p thành bäo thÕch cao c¤p #G(Bäo thÕch hşp thành c¥n thiªt t¯i thi¬u là 3 viên)")
	...
	if event == "UI_COMMAND" and tonumber( arg0 ) == 19810424 then	-- ²ÄÁÏºÏ³É
		...
		Materalcompose_SuccessValue : SetText("#cFF0000TÖ l® thành công");
		Current = 2
		Materalcompose_DragTitle : SetText("#gFF0FA0Nguyên li®u hşp thành")
...
function MaterialCompound_Update( pos0, pos1 )
	...
	if MaterialCompound_ItemInterface( bagPos ) ~= Current then
		PushDebugMessage( "LoÕi hình nguyên li®u không phù hşp" )
		return
	end
