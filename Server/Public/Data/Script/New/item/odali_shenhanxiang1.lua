...
function x002095_OnDefaultEvent( sceneId, selfId,targetId )
	...
	BeginEvent(sceneId)
		...
		AddText(sceneId,"  #b#u#e0000ffChào m×ng bÕn ðªn v¾i Thiên Long Bát Bµ. !#W")
			--AddNumText(sceneId, x002095_g_scriptId, "H°i phøc khí huyªt", 5, 106)
             AddNumText( sceneId, x002095_g_ScriptId, "#gFF8C00Nh§n Tâm Pháp 150",8,14 )
			 --AddNumText(sceneId,x990001_g_ScriptId,"Nh§n yªu quyªt phái Mµ Dung",1,325)
              AddNumText( sceneId, x002095_g_ScriptId, "#gFF8C00Nâng level t× 149 lên 150",8,3333 )
              --AddNumText( sceneId, x000146_g_ScriptId, "#cff0000Nh§n danh hi®u",5,1009 )
              AddNumText( sceneId, x002095_g_ScriptId, "#b#K#c993366Nh§n Vàng",5,10 )
	       --AddNumText( sceneId, x002093_g_ScriptId, "#b#GNh§n hi®u Ñng trùng lâu",5,101 )
              ...
              AddNumText( sceneId, x000146_g_ScriptId, "#gFF7F00Nh§n 5000000k KNB",5,13 )
			  --AddNumText( sceneId, x000146_g_ScriptId, "#b#GNh§n 300000 DMP",5,8888 )
              AddNumText( sceneId, x002093_g_ScriptId, "#gFF7F00Nh§n buff",8,100 )
              --AddNumText( sceneId, x002095_g_ScriptId, "#b#GNh§n ði¬m t£ng",5,6666 )
              AddNumText( sceneId, x002095_g_scriptId, "#b#YXem S¯ L¥n CS", 2, 4242)
             AddNumText( sceneId, x002095_g_ScriptId, "#ea00000#bChuy¬n sinh",6,5555 )
              AddNumText( sceneId, x002095_g_ScriptId, "#GSØa l²i kinh nghi®m <0>",5,11 )
	EndEvent(sceneId)
...
if key  == 4242 then
		...
		BeginEvent(sceneId)
			AddText(sceneId,"S¯ l¥n #GChuy¬n Sinh#W cüa các ha tính t¾i hi®n tai là: #G"..num.." l¥n#W!")
			--AddText(sceneId,"T¯i ?a c?n l?i "..(200-num).." l¥n #Gcäi l?o hoàn ?°ng#W næa!")
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
...
if key == 5555 then --chuy¬n sinh
		BeginEvent( sceneId )
		AddText( sceneId, "#gF4F4F4BÕn có ch¡c là mu¯n chuy¬n sinh ?#r#cFF0000T× l¥n chuy¬n sinh 1->100 : Yêu c¥u c¤p ðµ #G130#r#cFF0000T× l¥n chuy¬n sinh 100->200 : Yêu c¥u c¤p ðµ #G149#r#YSau khi chuy¬n sinh giæ nguyên t¤t cä. Sau khi chuy¬n sinh c¤p ðµ tr· v« #G30." )
		AddNumText(sceneId, x002095_g_scriptId, "#gF4F4F4Có, tôi mu¯n chuy¬n sinh", 6, 5566 )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
end
if key == 5566 then
...
if key == 58 then
	...
	if key == 13 then
		...
		if key == 14 then
	BeginEvent( sceneId )
		AddText( sceneId, "Hãy ch÷n môn phái bÕn mu¯n gia nh§p. #r#GSau khi gia nh§p môn phái bÕn s¨ ðßþc tâm pháp 150 bao g°m cä Bí T¸ch. #r#YLßu ý: Nªu các hÕ ðã chuy¬n sinh r°i mà sØ døng chÑc nång này s¯ ði¬m ti«m nång s¨ m¤t." )
	...
	if key == 20 then
		...
		LuaFnSetXinFaLevel(sceneId,selfId,77,150)
              local	nam	= LuaFnGetName( sceneId, selfId )
		BroadMsgByChatPipe( sceneId, selfId, "#ea00000#GChúc m×ng "..nam.." ðã nh§n tâm pháp 150 phái Tinh Túc !", 4 )
	...
	if key == 21 then
		...
		LuaFnSetXinFaLevel(sceneId,selfId,80,150)
              local	nam	= LuaFnGetName( sceneId, selfId )
		BroadMsgByChatPipe( sceneId, selfId, "#ea00000#GChúc m×ng "..nam.." ðã nh§n tâm pháp 150 phái Tiêu Dao !", 4 )
		 BroadMsgByChatPipe(sceneId, selfId, strText, 4)
	...
	if key == 22 then
		...
		LuaFnSetXinFaLevel(sceneId,selfId,72,150)
              local	nam	= LuaFnGetName( sceneId, selfId )
		BroadMsgByChatPipe( sceneId, selfId, "#ea00000#GChúc m×ng "..nam.." ðã nh§n tâm pháp 150 phái Thiªu Lâm !", 4 )
		SetMissionData(sceneId, selfId, MF_LINGQUYUANBAOMP1, day);
		BeginEvent( sceneId )
			AddText( sceneId, "#GBÕn ðã nh§n BUFF tâm pháp thành công. Chúc bÕn ch½i vui v¨ !" )
		EndEvent( sceneId )
	...
	if key == 23 then
		...
		LuaFnSetXinFaLevel(sceneId,selfId,79,150)
              local	nam	= LuaFnGetName( sceneId, selfId )
		BroadMsgByChatPipe( sceneId, selfId, "#ea00000#GChúc m×ng "..nam.." ðã nh§n tâm pháp 150 phái Thiên S½n !", 4 )
		 BroadMsgByChatPipe(sceneId, selfId, strText, 4)
		SetMissionData(sceneId, selfId, MF_LINGQUYUANBAOMP1, day);
		BeginEvent( sceneId )
			AddText( sceneId, "#GBÕn ðã nh§n BUFF tâm pháp thành công. Chúc bÕn ch½i vui v¨ !" )
	...
	if key == 24 then
		...
		LuaFnSetXinFaLevel(sceneId,selfId,78,150)
              local	nam	= LuaFnGetName( sceneId, selfId )
		BroadMsgByChatPipe( sceneId, selfId, "#ea00000#GChúc m×ng "..nam.." ðã nh§n tâm pháp 150 phái Thiên Long !", 4 )
	...
	if key == 25 then
		...
		LuaFnSetXinFaLevel(sceneId,selfId,76,150)
              local	nam	= LuaFnGetName( sceneId, selfId )
		BroadMsgByChatPipe( sceneId, selfId, "#ea00000#GChúc m×ng "..nam.." ðã nh§n tâm pháp 150 phái Nga My !", 4 )
		SetMissionData(sceneId, selfId, MF_LINGQUYUANBAOMP1, day);
		BeginEvent( sceneId )
			AddText( sceneId, "#GBÕn ðã nh§n BUFF tâm pháp thành công. Chúc bÕn ch½i vui v¨ !" )
		EndEvent( sceneId )
	...
	if key == 26 then
		...
		LuaFnSetXinFaLevel(sceneId,selfId,75,150)
              local	nam	= LuaFnGetName( sceneId, selfId )
		BroadMsgByChatPipe( sceneId, selfId, "#ea00000#GChúc m×ng "..nam.." ðã nh§n tâm pháp 150 phái Võ Ðang !", 4 )
		SetMissionData(sceneId, selfId, MF_LINGQUYUANBAOMP1, day);
		BeginEvent( sceneId )
			AddText( sceneId, "#GBÕn ðã nh§n BUFF tâm pháp thành công. Chúc bÕn ch½i vui v¨ !" )
		EndEvent( sceneId )
	...
	if key == 27 then
		...
		LuaFnSetXinFaLevel(sceneId,selfId,73,150)
              local	nam	= LuaFnGetName( sceneId, selfId )
		BroadMsgByChatPipe( sceneId, selfId, "#ea00000#GChúc m×ng "..nam.." ðã nh§n tâm pháp 150 phái Minh Giáo !", 4 )
		SetMissionData(sceneId, selfId, MF_LINGQUYUANBAOMP1, day);
		BeginEvent( sceneId )
			AddText( sceneId, "#GBÕn ðã nh§n BUFF tâm pháp thành công. Chúc bÕn ch½i vui v¨ !" )
		EndEvent( sceneId )
	...
	if key == 28 then
		...
		LuaFnSetXinFaLevel(sceneId,selfId,74,150)
              local	nam	= LuaFnGetName( sceneId, selfId )
		BroadMsgByChatPipe( sceneId, selfId, "#ea00000#GChúc m×ng "..nam.." ðã nh§n tâm pháp 150 phái Cái Bang !", 4 )
		SetMissionData(sceneId, selfId, MF_LINGQUYUANBAOMP1, day);
		BeginEvent( sceneId )
			AddText( sceneId, "#GBÕn ðã nh§n BUFF tâm pháp thành công. Chúc bÕn ch½i vui v¨ !" )
		EndEvent( sceneId )
...
function x002095_ZhuanSheng( sceneId, selfId, targetId )
	...
	if zhuanshengTimes >= 305 then
		BeginEvent( sceneId )
			AddText( sceneId, "HÕn chª 305 l¥n chuy¬n sinh, Trong Hôm Nay!!" )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
	...
	if zhuanshengTimes >= 150 then
		...
		if number<150 then
			BeginEvent( sceneId )
			AddText( sceneId, "#GCác hÕ c¥n thu th§p ðü #Y150 #Gviên #W#Long #Gð¬ có th¬ chuy¬n sinh." )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
			return -1
		else
	...
	BeginEvent(sceneId)
		...
		AddText( sceneId, "#WServer gi¾i hÕn s¯ l¥n chuy¬n sinh là #Y305, Trong Hôm Nay!!" )
	EndEvent(sceneId)
	DispatchMissionTips(sceneId,selfId)
	BeginEvent(sceneId)
		AddText(sceneId,"#GChúc m×ng, chuy¬n sinh l¥n #Y"..zhuanshengTimes.. " #Gthành công!!!")
	...
	DispatchEventList( sceneId, selfId, targetId )
	local 	nam	= LuaFnGetName( sceneId, selfId )
	local szMsg = format("#2#G#cFF0000Chúc m×ng #W#{_INFOUSR%s} #gFFFF00ðã chuy¬n sinh thành công ! #2",nam)
