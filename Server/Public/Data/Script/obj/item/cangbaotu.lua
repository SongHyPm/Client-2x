...
--							{sn=30402016, name="¸ßc¤p¼ÓËÙ¼¼ÄÜÊé"},
--							{sn=30505004, name="ÑÌ»¨"},
--							{sn=30505001, name="ËÙÐÐÐ¬"},
--							{sn=30701009, name="×øÆï: °×»¢"},
--							{sn=30701007, name="×øÆï: Â¹"}
--							}
...
function x300022_DiscoverGhoulMonster(sceneId, selfId)
	...
	BeginEvent(sceneId)		
		AddText(sceneId, "C¦n th§n! B÷n ÐÕo Mµ Ti¬u T£c");
	EndEvent(sceneId)
	DispatchMissionTips(sceneId,selfId)
...
--**********************************
--¸ù¾Ý³¡¾°Id ði¬m³ö¶ÔÓ¦toÕ ðµ ¹ÖÎïtoÕ ðµ ID
--**********************************
...
function x300022_DiscoverCorpseMonster(sceneId, selfId)
		...
		for i=1, 10 do
			...
			--ÖLinhÙ±£Ö¤TÕi Íæ¼ÒÉí±ß³öÏÖmµt cái¶á±¦ÂíÔô
			if i == 1 then
			...
			--Èç¹û¹ÖÎïtoÕ ðµ ×î´óµÈc¤p³¬¹ýÍæ¼Ò×î´óµÈc¤pÉÏÏÞ,Ôò¹ÖÎïµÈc¤pµÈÓÚÍæ¼Ò×î´óµÈc¤pÉÏÏÞ
...
--**********************************
--R½i vào trong kho báu
--**********************************
function x300022_DiscoverInstance(sceneId, selfId)
		BeginEvent(sceneId)
			AddText(sceneId, "R½i vào trong kho báu");
...
function x300022_OnDefaultEvent( sceneId, selfId, BagPos)
	...
	--ÐÆng c¤p chßa ðÕt 30 mà khinh su¤t mò vào kho báu e tính mÕng chÆng lành
	...
	--Èç¹ûTÕi Ê¹ÓÃ·¶Î§, ÔòËæ»ú´¥·¢ÒÔÏÂÊÂ¼þ
	...
	elseif ret < 80 then -- ðÕt ðßþcÎïÆ·
	...
	elseif ret < 95 then --R½i vào trong kho báu
		x300022_DiscoverInstance(sceneId, selfId)
	else --Ðøng phäi c½ quan
