...
function x300100_OnActivateOnce( sceneId, selfId )
	...
	if not costMoneyRet or costMoneyRet < 0 then
		x300100_NotifyFailTips(sceneId, selfId, "Kh¤u tr× ti«n th¤t bÕi.");
		return 0;
	end
	...
	local strText = ""
	-- zchw
	if (itemTblIndex == 30309186) or (itemTblIndex == 30309187) then --Èç¹ûĞúng»¹Í¯µ¤: Ô§Ñì
...
function x300100_CanOperate(sceneId, selfId)
	...
	if not petType or petType == 1 then
		x300100_NotifyFailTips(sceneId, selfId, "Biªn D¸ và trân thú ğ¶i thÑ 2 không th¬ tiªn hành thao tác này.");
		return 0;
	end
