...
--½Å±¾ºÅ
 x335099_g_scriptId = 335099 --ÁÙÊ±Ð´Cái này ,ÕæÕýÓÃtoÕ ðµ Ê±ºòmµt ¶¨Òª¸Ä.
...
--×¢: Á½cáiÆïÊõ»ámµt cái¾Í¿ÉÒÔÆïÁË
 x335099_g_Equitation1 = 1130 --C¥n toÕ ðµ ÆïÊõ
 x335099_g_Equitation2 = -1 --C¥n toÕ ðµ ÆïÊõ
--Ð§¹ûtoÕ ðµ ID
 x335099_g_Impact1 = 4755 --ÁÙÊ±Ð´Cái này 
 x335099_g_Impact2 = -1 --²»ÓÃ

--**********************************
--ÊÂ¼þ½»»¥Èë¿Ú
--**********************************
function  x335099_OnDefaultEvent( sceneId, selfId, bagIndex )
...
--**********************************
function  x335099_IsSkillLikeScript( sceneId, selfId)
...
--**********************************
function  x335099_CancelImpacts( sceneId, selfId )
	nRet = 0;
	nRet = LuaFnCancelSpecificImpact(sceneId, selfId,  x335099_g_Impact1)
	if(0<nRet) then
		return 1;
	end
	return 0;
...
--**********************************
function  x335099_OnConditionCheck( sceneId, selfId )
	--Ð£ÑéÊ¹ÓÃtoÕ ðµ ÎïÆ·
	--if(1~=LuaFnVerifyUsedItem(sceneId, selfId)) then
		--return 0
	--end
	--Á½cáiÆïÊõ,»ámµt cái¾Í¿ÉÒÔÁË
	if(-1~= x335099_g_Equitation1) then
		if(1==LuaFnHaveSpecificEquitation(sceneId, selfId,  x335099_g_Equitation1)) then
			return 1
		end
	end
	if(-1~= x335099_g_Equitation2) then
		if(1==LuaFnHaveSpecificEquitation(sceneId, selfId,  x335099_g_Equitation2)) then
			return 1
		end
	end
...
--**********************************
function  x335099_OnDeplete( sceneId, selfId )
	return 1; --Æï³Ë²»ÏûºÄ
end
...
--**********************************
function  x335099_OnActivateOnce( sceneId, selfId )
	if(-1~= x335099_g_Impact1) then
		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId,  x335099_g_Impact1, 0)
	end
...
--×¢: ÕâÀïÐúng¼¼ÄÜÉúÐ§mµt ´ÎtoÕ ðµ Èë¿Ú
--**********************************
function  x335099_OnActivateEachTick( sceneId, selfId)
