...
--5097.lua
------------------------------------------------------------------------------------------
--½ð÷ëÎèÊ¨toÕ ðµ Ä¬ÈÏ½Å±¾

--½Å±¾ºÅ
 x335097_g_scriptId = 335097 --ÁÙÊ±Ð´Cái này ,ÕæÕýÓÃtoÕ ðµ Ê±ºòmµt ¶¨Òª¸Ä.
...
--×¢: Á½cáiÆïÊõ»ámµt cái¾Í¿ÉÒÔÆïÁË
 x335097_g_Equitation1 = 1128 --C¥n toÕ ðµ ÆïÊõ
 x335097_g_Equitation2 = -1 --C¥n toÕ ðµ ÆïÊõ
--Ð§¹ûtoÕ ðµ ID
 x335097_g_Impact1 = 4753 --ÁÙÊ±Ð´Cái này 
 x335097_g_Impact2 = -1 --²»ÓÃ

--**********************************
--ÊÂ¼þ½»»¥Èë¿Ú
--**********************************
function  x335097_OnDefaultEvent( sceneId, selfId, bagIndex )
...
--**********************************
function  x335097_IsSkillLikeScript( sceneId, selfId)
...
--**********************************
function  x335097_CancelImpacts( sceneId, selfId )
	nRet = 0;
	nRet = LuaFnCancelSpecificImpact(sceneId, selfId,  x335097_g_Impact1)
	if(0<nRet) then
		return 1;
	end
	return 0;
...
--**********************************
function  x335097_OnConditionCheck( sceneId, selfId )
	--Ð£ÑéÊ¹ÓÃtoÕ ðµ ÎïÆ·
	--if(1~=LuaFnVerifyUsedItem(sceneId, selfId)) then
		--return 0
	--end
	--Á½cáiÆïÊõ,»ámµt cái¾Í¿ÉÒÔÁË
	if(-1~= x335097_g_Equitation1) then
		if(1==LuaFnHaveSpecificEquitation(sceneId, selfId,  x335097_g_Equitation1)) then
			return 1
		end
	end
	if(-1~= x335097_g_Equitation2) then
		if(1==LuaFnHaveSpecificEquitation(sceneId, selfId,  x335097_g_Equitation2)) then
			return 1
		end
	end
...
--**********************************
function  x335097_OnDeplete( sceneId, selfId )
	return 1; --Æï³Ë²»ÏûºÄ
end
...
--**********************************
function  x335097_OnActivateOnce( sceneId, selfId )
	if(-1~= x335097_g_Impact1) then
		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId,  x335097_g_Impact1, 0)
	end
...
--×¢: ÕâÀïÐúng¼¼ÄÜÉúÐ§mµt ´ÎtoÕ ðµ Èë¿Ú
--**********************************
function  x335097_OnActivateEachTick( sceneId, selfId)
