...
function x334834_OnConditionCheck( sceneId, selfId )
	...
	if LuaFnGetPetLevelByGUID( sceneId, selfId, petGUID_H, petGUID_L ) >= 70 then
		BeginEvent( sceneId )
			AddText( sceneId, "ÄútoÕ ğµ ³èÎï³¬¹ı 70 c¤p,ÇëÊ¹ÓÃ¸ßc¤p»¹Í¯µ¤½øĞĞ»¹Í¯." )
		EndEvent( sceneId )
	...
	if not petDataID or petDataID < 0 then
		BeginEvent( sceneId )
			AddText( sceneId, "ÎŞ·¨¶ÔÖ¸¶¨ÕäÊŞ½øĞĞ»¹Í¯." )
		EndEvent( sceneId )
		DispatchMissionTips( sceneId, selfId )
		return 0
	end
	...
	if not petTakeLevel or petTakeLevel < 1 then
		BeginEvent( sceneId )
			AddText( sceneId, "ÎŞ·¨Ê¶±ğÕäÊŞtoÕ ğµ Ğ¯´øµÈc¤p." )
		EndEvent( sceneId )
		DispatchMissionTips( sceneId, selfId )
		return 0
	end
