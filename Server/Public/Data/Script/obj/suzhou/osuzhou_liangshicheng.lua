...
-- ñÒÉÙÎ¢,2008.5.29.¼«Æ·×°±¸·Å³ö.Á½cái×°±¸: 10422016--ÖØÂ¥½ä	10423024--ÖØÂ¥Óñ
{n=9100,id=10422016},
{n=9100,id=10423024},
}
...
-- ñÒÉÙÎ¢,2008.5.29.¼«Æ·×°±¸·Å³ö.Á½cái¶mµt »C¥n toÕ ðµ ÎïÆ·: 20310101--Trùng lâu chi l®	20310102--Trùng lâu chi mang
{n=9,id=20310101,num=1,str="Trùng lâu chi l®"},
{n=10,id=20310102,num=1,str="Trùng lâu chi mang"},

}

x001067_g_Stone2ID = 20309018 --ÐÂÔö¶þc¤pÍòLinhÊ¯(°ó¶¨) 
--**********************************
--ÊÂ¼þÁÐ±í
--**********************************
function x001067_UpdateEventList( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		AddNumText( sceneId, x001067_g_MenPaiTaoScriptId, "Ð±i l¤y bµ th¶i trang môn phái", 6, 846 )
		
		AddNumText( sceneId, x001067_g_ScriptId, "Ð±i l¤y bµ trang b¸ c¤p 18", 6, 1000 )
		AddNumText( sceneId, x001067_g_ScriptId, "Ð±i l¤y bµ trang b¸ c¤p 30", 6, 2000 )
		AddNumText( sceneId, x001067_g_ScriptId, "Ð±i l¤y bµ trang b¸ c¤p 40", 6, 3000 )
		AddNumText( sceneId, x001067_g_ScriptId, "Ð±i l¤y bµ trang b¸ c¤p 50", 6, 4000 )
		AddNumText( sceneId, x001067_g_ScriptId, "Ð±i l¤y bµ trang b¸ c¤p 60", 6, 5000 )
		AddNumText( sceneId, x001067_g_ScriptId, "Ð±i l¤y bµ trang b¸ c¤p 70", 6, 6000 )
		AddNumText( sceneId, x001067_g_ScriptId, "Ð±i l¤y bµ trang b¸ c¤p 80", 6, 7000 )
		AddNumText( sceneId, x001067_g_ScriptId, "Ð±i l¤y bµ trang b¸ c¤p 90", 6, 8000 )

		AddNumText( sceneId, x001067_g_ScriptId, "#GGi¾i thi®u ð±i trùng lâu", 11, 10000 )		
		-- ñÒÉÙÎ¢,2008.5.29.¼«Æ·×°±¸·Å³ö.
		AddNumText( sceneId, x001067_g_ScriptId, "Ð±i Trùng Lâu", 6, 9000 )
...
function x001067_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if nNumText == 1000 or nNumText == 2000 or nNumText == 3000 or nNumText == 4000 or nNumText == 5000 or nNumText == 6000 or nNumText == 7000 or nNumText == 8000  then
		BeginEvent(sceneId)
			...
			if nNumText > 2000  then
				AddNumText(sceneId, x001067_g_ScriptId, "Ð±i l¤y hµ uy¬n", 0, nNumText+500)
				if nNumText > 4000  then
					AddNumText(sceneId, x001067_g_ScriptId, "Ð±i l¤y s¤n kiên", 0, nNumText+600)
				end
			end
			AddNumText( sceneId, x001067_g_ScriptId, "R¶i ði..", 0, 0 )
		EndEvent(sceneId)
	...
	if nNumText > 1000 and nNumText < 9000  then
		BeginEvent(sceneId)
			...
			local szStr = "  Mu¯n nh§n ðßþc bµ trang b¸ này, ngß½i c¥n phäi ðßa cho ta\"" .. x001067_g_StoneList[nLevel].str 
										.. "\" \"".. tostring(x001067_g_StoneList[nLevel].num) .. " Cái"
	...
	-- ñÒÉÙÎ¢,2008.5.29.¼«Æ·×°±¸·Å³ö.Ìí¼ÓÁ½cái°´Å¥toÕ ðµ ´¦ÀíÊÂ¼þ
...
function x001067_OnMissionSubmit( sceneId, selfId, targetId, missionScriptId, selectRadioId )
  ...
  if nLevel == 2 then 
	...
	-- ¼ì²éÐúng²»ÐúngÓÐ×ã¹»toÕ ðµ Ê¯Í·¿ÉÒÔ¿Û³ý
   ...
   --Ôö¼Ó¶þc¤p°ó¶¨ÍòLinhÊ¯ÅÐ¶Ï
	if  nLevel == 2 then
     nItemBagIndexStone = GetBagPosByItemSn(sceneId, selfId, x001067_g_Stone2ID)
     if nItemBagIndexStone == -1 then --Ã»ÓÐ¶þc¤p°ó¶¨ÍòLinhÊ¯
  ...
  if nItemBagIndexStone ~= -1 then
	...
	local bDelOk = 0
	-- É¾³ýÏà¹ØtoÕ ðµ Ê¯Í·
	if nLevel == 2 and Stone2Count > 0 then --ÓÅÏÈ¿Û³ý°ó¶¨toÕ ðµ ÍòLinhÊ¯
	...
	if bDelOk < 1  then
		...
		-- ñÒÉÙÎ¢,2008.5.29.¼«Æ·×°±¸·Å³ö.ÕâÁ½cái¼«Æ·×°±¸ÎÞ·¨´ò¿×,Ç¿ÖÆ¿ÌÃú
		...
		elseif randMessage == 2 then
			...
			message = format("#W#{WLS_00}#Y%d# quä#W#{_INFOMSG%s}#I khen r¢ng: \#W#{_INFOUSR%s}#{WLS_01}#{_INFOMSG%s}#{WLS_02}", x001067_g_StoneList[nLevel].num, szTransferStone, LuaFnGetName(sceneId, selfId), szTransferEquip);
		end
