...
function x805014_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	elseif GetNumText() == 4 then
		...
		BeginEvent(sceneId)
			...
			AddNumText( sceneId, x805014_g_scriptId, "HuÖ", 8, 17 )
		EndEvent(sceneId)
		DispatchEventList( sceneId, selfId, targetId )
...
function x805014_BingZhenXiGua(sceneId, selfId, targetId)
	...
	x805014_NotifyFailTips(sceneId, selfId, "Các hÕ nh§n ðßþc 1 miªng Dßa h¤u ß¾p lÕnh");
	
	local szTransferItem = GetItemTransfer(sceneId, selfId, 0);
	local selfName = LuaFnGetName(sceneId, selfId);
	local strChatMessage = "#{_INFOUSR"..selfName.."}#cff99cc h¾n h· t× trong tay#G Chu Thª hæu[129, 100]#cff99cc l¤y 1 miªng#Y#{_INFOMSG" .. szTransferItem .. "}#cff99cc, ån 1 miªng vào mát t× ð¥u xu¯ng ðªn bàn chân!";
