...
function x805008_CheckFavorOfGuild( sceneId, selfId )
	...
	local OldDay = GetMissionData( sceneId, selfId, MD_FAVOROFGUILD_LASTTIME ) --È¡³öÉÏ´ÎLînh °ïÅÉ¹Ø»³toÕ ðµ Ê±¼ä

	if( CurDay <= OldDay ) then
	    return 3  --¾àÉÏ´ÎLînh Ê±¼ä²»×ã1Ìì
	end
...
function x805008_SetFavorOfGuild( sceneId, selfId )
	...
	if( Point > 0 ) then
        ...
        --CitySetAttr( sceneId, selfId, 6, GuidPoint - 1 )   --ÏûºÄmµt  ði¬m°ï¹±
        CityChangeAttr( sceneId, selfId, 6, -1 )   --ÏûºÄmµt  ði¬m°ï¹±

        SetMissionData( sceneId, selfId, MD_FAVOROFGUILD_LASTTIME, CurDay ) --ÉèÖÃLînh °ïÅÉ¹Ø»³toÕ ðµ Ê±¼ä
...
function x805008_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	elseif GetNumText() == 12 then
	    local FavorCode = x805008_CheckFavorOfGuild( sceneId, selfId )
	    if( 0 == FavorCode ) then  --Thöa mãnÌõ¼þ
	        x805008_SetFavorOfGuild( sceneId, selfId )
	    elseif( 1 == FavorCode ) then  --²»Ðúng±¾°ï
	        BeginEvent(sceneId)
			    AddText( sceneId, "#{FAVOROFGUILD_NOTMEMBER}" )
		    EndEvent(sceneId)
		    DispatchEventList( sceneId, selfId, targetId )
	    elseif( 2 == FavorCode ) then  --°ï¹±²»¹»1 ði¬m
	        BeginEvent(sceneId)
			    AddText( sceneId, "#{FAVOROFGUILD_POINTNOTENOUGH}" )
		    EndEvent(sceneId)
		    DispatchEventList( sceneId, selfId, targetId )
	    elseif( 3 == FavorCode ) then  --²»×ãLînh Ê±¼ä 3
	        BeginEvent(sceneId)
