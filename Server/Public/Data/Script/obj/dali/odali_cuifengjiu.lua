...
function x002026_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if arg == 1000 then		--Tr· v«ÃÅÅÉ
		if id < 0 or id >= 9 then
			x002026_MsgBox( sceneId, selfId, targetId, "  Các hÕ chßa gia nh§p môn phái nào." )
		else
	...
	if arg == 1000 then		--·µ»ØÃÅÅÉ
		if id < 0 or (id == 9 and LuaFnGetXinFaLevel(sceneId,selfId,64) <= 0) then --Add by Sói
			x002026_MsgBox( sceneId, selfId, targetId, "Ngß½i vçn chßa gia nh§p môn phái." )
		elseif id == 9 and LuaFnGetXinFaLevel(sceneId,selfId,64) > 0 then --Add by Sói
