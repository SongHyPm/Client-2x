...
function x805019_OnDefaultEvent( sceneId, selfId,targetId )
	...
	if(guildid ~= cityguildid) then
		BeginEvent(sceneId)
			strText = "    Giá cá bang s· hæu huynh ð® ngã ðô kí ð¡c danh tñ nh¤t khán tñu tri ðÕo các hÕ b¤t th¸ ngã môn bang ðích"
	...
	BeginEvent(sceneId)
		...
		AddText(sceneId,strText);
		AddNumText( sceneId, x805019_g_scriptId, "Gi¾i thi®u sß½ng phòng", 8, 1 )
--¹Ø±ÕÕäÊÞÉáÀû×Ó¼°°ïÖú£¬Ìí¼Ó¼ÄÁ·ÕäÊÞ£¬ by zhangqiang 2009.6.19
--		AddNumText( sceneId, x805019_g_scriptId, "Á¶ÖÆÕäÊÞÉáÀû×Ó", 6, 2 )
--		AddNumText( sceneId, x805019_g_scriptId, "¹ØÓÚÕäÊÞ¿ìËÙÉý¼¶", 11, 3 )
		AddNumText( sceneId, x805019_g_scriptId, "Kí luy®n trân thú", 6, 4 )
...
--		DispatchUICommand(sceneId, selfId, 8050191);
	elseif GetNumText() == 4 then 
		BeginEvent(sceneId)
			AddText( sceneId, "Giá cá công nång tÑc tß½ng khai phóng" )	
		EndEvent(sceneId)
		DispatchEventList( sceneId, selfId, targetId )
	end
end
...
function x805019_PetShelizi_Done(sceneId, selfId, PetGuidH, PetGuidL)
	...
	--ÊÇ·ñ´óÓÚµÈÓÚ30¼¶£¬ÊÇ·ñÊÇ±¦±¦£¬¶þ´ú»ò±äÒì
