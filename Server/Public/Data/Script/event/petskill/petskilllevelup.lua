-- Thång c¤p kÛ nång trân thú

-- ½Å±¾ºÅ
x311112_g_ScriptId = 311112;
x311112_g_NumText_Main = 1					-- ½ÓÈÎÎñtoÕ ðµ Ñ¡Ïî
...
function x311112_OnDefaultEvent(sceneId, selfId, targetId)
	...
	if numText == x311112_g_NumText_Main then
		...
		DispatchUICommand( sceneId, selfId, 19823 )	--µ÷ÓÃThång c¤p kÛ nång trân thú½çÃæ
	end
...
function x311112_PetSkillLevelup( sceneId, selfId, petHid, petLid, skillindex, ItemIndexInBag )
	...
	if checkAvailable and checkAvailable == 1 then
		...
		if itemid ~= ConsumeGoodsID then --Èç¹ûConsumeGoodsIDÎª0Ë ði¬m÷¿ÉÄÜÐúng²ß»®Íü¼ÇÅäÏûºÄÎïÆ·IDÁË,Ò²²»ÄÜÍ¨¹ý
		...
		x311112_NotifyFailTips(sceneId, selfId, "Thång c¤p thành công")
...
function x311112_PetMedicineHC( sceneId, selfId, ItemIndex1, ItemIndex2, ItemIndex3, ItemIndex4, ItemIndex5)
	...
	-- ²»ÔÊÐíÓÐÖØ¸´toÕ ðµ ItemIndex1³öÏÖ added by dun.liu 2009.2.5
	...
	--Ðúng·ñ½ðÇ®×ã¹»,Îª0Ë ði¬m÷ÐúngÃ»ÓÐ½ðÇ®ÒªÇó
	local havemoney = GetMoney(sceneId, selfId)
	local haveJiaoZi = GetMoneyJZ(sceneId, selfId)
	...
	--¿Û³ý½ðÇ®,Îª0Ë ði¬m÷ÐúngÃ»ÓÐ½ðÇ®ÒªÇó
	if money > 0 then
