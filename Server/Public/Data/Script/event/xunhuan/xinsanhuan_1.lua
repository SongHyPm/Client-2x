...
x050220_g_IsSmallMonster = 1														-- 1 ºÅË÷Òý: É±ÐþÀ×ÆÂÍÁ·ËtoÕ ðµ ÊýÁ¿
...
function x050220_AcceptEnterCopyScene( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050220_g_MissionId ) > 0 then					-- ÓÐÈÎÎñ²Å¿ÉÒÔ×ßÕâÀï
		...
		if bRetSceneID > 0 then
			x050220_NotifyFailTips( sceneId, selfId, "D¸ch chuy¬n thành công!" )
		...
		if otherflag == 0 then --Áímµt cáibossÃ»ÓÐËÀ
			for i=0, nMonsterNum-1 do
				...
				if GetName( sceneId, MonsterId ) == x050220_g_NiuQi then
					--CallScriptFunction((200060), "Paopao",sceneId, x050220_g_NiuQi, "ÐþÀ×ÆÂ", "´ó¸ç, huynh ð®ÌæÄã±¨³ð!")
					MonsterTalk(sceneId,MonsterId,"Huy«n Lôi Sß¶n Núi", "ÐÕi ca, ti¬u ð® báo thù cho huynh!")
				end
			end
		else
			for i=0, nMonsterNum-1 do
		...
		if otherflag == 0 then --Áímµt cáibossÃ»ÓÐËÀ
			for i=0, nMonsterNum-1 do
				...
				if GetName( sceneId, MonsterId ) == x050220_g_NiuQu then
					--CallScriptFunction((200060), "Paopao",sceneId, x050220_g_NiuQu, "ÐþÀ×ÆÂ", " huynh ð®,´ó¸çÌæÄã±¨³ð!")
					MonsterTalk(sceneId,MonsterId,"Huy«n Lôi Sß¶n Núi", " Huynh ð®, ðÕi ca báo thù cho ngß½i!")
				end
			end
		else
			for i=0, nMonsterNum-1 do
...
--				LuaFnSetCopySceneData_Param( sceneId, 11, TickCount )
		...
		elseif TickCount >= 180 and monsternum >= 60 and iskillNiuQu == 1 and iskillNiuQi == 1 and isBossWang == 0 then
			...
			MonsterTalk(sceneId,monsterID,"Huy«n Lôi Sß¶n Núi","Kë nào dám xông vào n½i ðây hãy sÇn sàng nh§n l¤y cái chªt ði!")
	...
	if x050220_CheckSubmit( sceneId, selfId ) == 1 then
		...
		if LuaFnDelAvailableItem( sceneId, selfId, x050220_g_huangjinzhilian, 1 ) > 0 then
			...
			if ret > 0 then
				...
				DelMission( sceneId, selfId, x050220_g_MissionId )
				-- ÈÎÎñ»mµt î¶¯Í³¼Æ
				LuaFnAuditQuest(sceneId, selfId, "Nhi®m vø liên hoàn Lâu Lan, Hoàn Kim Chi Liên")
			else
