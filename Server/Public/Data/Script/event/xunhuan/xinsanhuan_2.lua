...
x050221_g_BossTalk = {"Kë nào dám qu¤y r¥y ta nghï ng½i?", --bossËµ»°
"Ðây không phäi n½i các ngß½i mu¯n là ðªn!",
"Tñ ti®n xông vào lãnh ð¸a cüa ta chï có ðß¶ng chªt!",
"LÕi có kë ðªn ðây ð¬ ch¸u chªt!",
"Ha ha, r¯t cøc lÕi có th¬ luy®n t§p tay chân!"}

x050221_g_YaoWangTalk = "Kë nào dám hÕ sát hài nhi cüa ta, mau t¾i ðây nh§n l¤y cái chªt!"			-- bossH°ng Cúc Yêu Vß½ngËµ»°
...
function x050221_AcceptEnterCopyScene( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050221_g_MissionId ) > 0 then					-- ÓÐÈÎÎñ²Å¿ÉÒÔ×ßÕâÀï
		...
		if bRetSceneID > 0 then
			x050221_NotifyFailTips( sceneId, selfId, "D¸ch chuy¬n thành công!" )
...
function x050221_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	...
	if x050221_CheckSubmit( sceneId, selfId ) == 1 then
		...
		DelMission( sceneId, selfId, x050221_g_MissionId )
		-- ÈÎÎñ»mµt î¶¯Í³¼Æ
		LuaFnAuditQuest(sceneId, selfId, "Lâu LanÁ¬»·ÈÎÎñÐþ·ðÖé")
	end
end
