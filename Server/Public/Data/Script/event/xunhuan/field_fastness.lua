...
x050102_g_MissionTarget = "    Tô Châu #{_INFOAIM62,162,1, "..x050102_g_Name.."} yêu c¥u các hÕ giªt chªt phï ğ± ğ¥u lînh Cát Vinh.#r    #{FQSH_090206_01}"									-- Møc tiêu nhi®m vø
...
x050102_g_BroadcastMsg = {
	"#Y"..x050102_g_Name .. ": #cff99cc Báo cho m÷i ngß¶i 1 thông tin t¯t lành, tên thü lînh danh tiªng x¤u xa cüa phï ğ° là #{_BOSS47}#cff99cc, hôm nay cu¯i cùng cûng ğã phäi ğ«n tµi, ngß¶i giªt chªt h¡n chính là #{_INFOUSR$N}#cff99cc. m÷i ngß¶i cùng v² tay nhé!",
	"#Y"..x050102_g_Name .. ": #cff99cc chúng ta hãy cùng chúc m×ng. #{_BOSS47}#cff99cc ğã không th¬ làm hÕi m÷i ngß¶i næa r°i, h¡n ğã chªt trong tay cüa #{_INFOUSR$N}#cff99cc, m÷i ngß¶i hãy cùng hoan hô nhé!",
	"#Y"..x050102_g_Name .. ": #{_BOSS47}#cff99cc  chªt r°i! B¡t ğ¥u t× hôm nay, chúng ta không c¥n phäi s¯ng trong trÕng thái th¤p thöm lo sş næa! Hãy ca ngşi v¸ anh hùng cüa chúng ta: #{_INFOUSR$N}#cff99cc các hÕ quä th§t r¤t tài giöi!"
}
...
function x050102_OnAccept( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050102_g_MissionId ) == 0 then					-- Ã»ÓĞÈÎÎñ²Å¿ÉÒÔ×ßÕâÀï
		...
		if LuaFnDelAvailableItem( sceneId, selfId, x050102_g_Mail, 1 ) == 0 then
			...
			x050102_NotifyFailBox( sceneId, selfId, targetId, "    C¥n có #{_ITEM" .. x050102_g_Mail .."}" )
			return
		end
...
function x050102_AcceptEnterCopyScene( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050102_g_MissionId ) > 0 then					-- ÓĞÈÎÎñ²Å¿ÉÒÔ×ßÕâÀï
		...
		if not teamMemberCount or not nearMemberCount or teamMemberCount ~= nearMemberCount then
			x050102_NotifyFailBox( sceneId, selfId, targetId, "    Các hÕ có nhóm viên không · g¥n ğây" )
			return
		end
		...
		if nearMemberCount < x050102_g_LimitMembers then
			x050102_NotifyFailBox( sceneId, selfId, targetId, "    Lû ğÕo t£c này không phäi v×a ğâu, ngß½i c¥n phäi có 1 s¯ bän lînh và 1 s¯ trş thü, ta m¾i yên tâm. (trong nhóm t¯i thi­u c¥n phäi có "..x050102_g_LimitMembers.." nhân v§t t× c¤p 30 tr· lên)" )
			return
		end
...
function x050102_OnCopySceneReady( sceneId, destsceneId )
	...
	if validmembercount < x050102_g_LimitMembers then
		x050102_NotifyFailTips( sceneId, leaderObjId, "    Lû ğÕo t£c này không phäi v×a ğâu, ngß½i c¥n phäi có 1 s¯ bän lînh và 1 s¯ trş thü, ta m¾i yên tâm. (trong nhóm t¯i thi­u c¥n phäi có "..x050102_g_LimitMembers.." nhân v§t t× c¤p 30 tr· lên)" )
		return
	end
