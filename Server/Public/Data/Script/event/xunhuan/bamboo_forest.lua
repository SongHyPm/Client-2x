...
x050101_g_MissionTarget = "    "..x050101_g_Name.." · Tô Châu #{_INFOAIM251,108,1, "..x050101_g_Name.."} yêu c¥u các hÕ giªt chªt 80 con Dã Hùng và H°ng Hùng Vß½ng.#r    #{FQSH_090206_01}"						-- Møc tiêu nhi®m vø
...
x050101_g_BroadcastMsg = {
	"#Y"..x050101_g_Name .. ": #W#{_INFOUSR$N}#cff99cc ğÕi hi®p th§t là mÕnh, mµt quy«n ğã ğánh b©p #{_BOSS46}#cff99cc ğã có #{_INFOUSR$N}#cff99cc ğÕi hi®p ·, tên trµm nào dám ngang ngßşc?",
	"#Y"..x050101_g_Name .. ": #W#{_INFOUSR$N}#cff99cc ğÕi hi®p th§t là khá, quét sÕch r×ng trúc Tô Châu. Mµt tr§n giáng xu¯ng ğã ğ¶i, không có #{_BOSS46}#cff99cc không d©p n±i",
	"#Y"..x050101_g_Name .. ": #W#{_INFOUSR$N}#cff99cc ğÕi hi®p th§t là lşi hÕi, danh tiªng nghîa hi®p lßu danh ngàn ğ¶i. Võ công càng không phäi nói, phàm là #{_BOSS46} #cff99ccğ«u vŞ ğ¥u hªt"
}
...
function x050101_AcceptEnterCopyScene( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050101_g_MissionId ) > 0 then					-- ÓĞÈÎÎñ²Å¿ÉÒÔ×ßÕâÀï
		...
		if not teamMemberCount or not nearMemberCount or teamMemberCount ~= nearMemberCount then
			x050101_NotifyFailBox( sceneId, selfId, targetId, "    Các hÕ có nhóm viên không · g¥n ğây" )
			return
		end
		...
		if nearMemberCount < x050101_g_LimitMembers then
			x050101_NotifyFailBox( sceneId, selfId, targetId, "    Trong r×ng trúc g¤u dæ r¤t nhi«u ít nh¤t các ngß½i phäi có "..x050101_g_LimitMembers.." ngß¶i ğ°ng hành, ta m¾i yên tâm ğ¬ các ngß½i ği" )
			return
		end
...
function x050101_OnCopySceneReady( sceneId, destsceneId )
	...
	if validmembercount < x050101_g_LimitMembers then
		x050101_NotifyFailTips( sceneId, leaderObjId, "    Trong r×ng trúc g¤u dæ r¤t nhi«u, ít nh¤t các ngß½i phäi có "..x050101_g_LimitMembers.." ngß¶i ğ°ng hành, ta m¾i yên tâm ğ¬ các ngß½i ği, trong nhóm c¥n có "..x050101_g_LimitMembers.." nhân v§t ít nh¤t t× c¤p 30 tr· lên)" )
		return
	end
