...
x050100_g_MissionTarget = "    "..x050100_g_Name.." · Tô Châu #{_INFOAIM62,162,1, "..x050100_g_Name.."} yêu c¥u các hÕ trong vòng 30 phút phäi dø và giªt chªt Dß Ğµc.#r    #{FQSH_090206_01}"						-- Møc tiêu nhi®m vø
...
x050100_g_BroadcastMsg = {
	"#Y"..x050100_g_Name..": #{_BOSS45}#cff99cc ğã chªt! H¡n ğã b¸ anh hùng cüa chúng ta #{_INFOUSR$N}#cff99cc tiêu di®t! Kë nÕp mÕng tiªp theo là ai? #{_BOSS46} hay là #{_BOSS47}. Ha ha ha!",
	"#Y"..x050100_g_Name..": #cff99ccAnh hùng cüa chúng ta #{_INFOUSR$N}#cff99cc, t× #Gbiên gi¾i T¯ng Liêu#cff99cc mang tin vui t¾i! Tên th± phï ác ôn #{_BOSS45}#cff99cc này, ğã b¸ tr× khØ!",
	"#Y"..x050100_g_Name..": #cff99ccM÷i ngß¶i mau ra ğây xem v¸ anh hùng cüa chúng ta mµt nhát kiªm ğã giªt chªt #{_BOSS45}! #{_INFOUSR$N}#cff99cc mµt huy«n thoÕi s¯ng, hi®p sî chiªn ğ¤u, ğÕi hi®p!"
}
...
function x050100_AcceptEnterCopyScene( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050100_g_MissionId ) > 0 then					-- ÓĞÈÎÎñ²Å¿ÉÒÔ×ßÕâÀï
		...
		if nearMemberCount < x050100_g_LimitMembers then
			x050100_NotifyFailBox( sceneId, selfId, targetId, "    Lû ğÕo t£c này không phäi v×a ğâu, ngß½i c¥n phäi có 1 s¯ bän lînh và 1 s¯ trş thü, ta m¾i yên tâm. (trong nhóm t¯i thi­u c¥n phäi có "..x050100_g_LimitMembers.." nhân v§t t× c¤p 30 tr· lên)" )
			return
		end
...
function x050100_OnCopySceneReady( sceneId, destsceneId )
	...
	if validmembercount < x050100_g_LimitMembers then
		x050100_NotifyFailTips( sceneId, leaderObjId, "    Lû ğÕo t£c này không phäi v×a ğâu, ngß½i c¥n phäi có 1 s¯ bän lînh và 1 s¯ trş thü, ta m¾i yên tâm. (trong nhóm t¯i thi­u c¥n phäi có "..x050100_g_LimitMembers.." nhân v§t t× c¤p 30 tr· lên)" )
		return
	end
