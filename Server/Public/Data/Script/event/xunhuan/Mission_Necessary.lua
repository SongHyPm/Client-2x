--!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
--¸Ã½Å±¾ÎÄ¼şÖ»¹©Nhi®m vø sß môn¶ø×ö, ÆäËûÈÎÎñÈçÒªÒıÓÃ, ºó¹û×Ô¸º!
--!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
x500501_g_LingShouDanId = {30503034, 30503043, 30503052, 30503061} --Linh Thúµ¤ID

x500501_g_strHelpFinishedText = "  Í¬ÃÅÓĞÄÑ,Àíµ±ÏàÖú,Cüa ngß½i ÈÎÎñÎÒÒÑ¾­°²ÅÅÆäËûÍ¬ÃÅÈ¥Íê³ÉÁË.#r#GHoàn t¤t nhi®m vø!#W"
...
function x500501_ACT_Duanwu( sceneId, selfId, iDayHuan ) --¶ËÎç»î¶¯
	...
	if AddRet > 0 then
		...
		local Gemable = random(100)  -- 1/5 ğÕt ğßşc±¦Ê¯
		if Gemable > 20 then
			return
		end
		...
		if Duanwu_GemCount >= Duanwu_MaxGemCount then
			return     --±¦Ê¯ÊıÁ¿³¬¹ıÁËÃ¿ÌìËÍ³ötoÕ ğµ ×î´óÊıÁ¿
		end
		...
		if GemRet > 0 then
			...
			local strformat	= "#{_INFOUSR%s}#Trong hoÕt ğµng Tiªt Ğoan Ng÷, do hòan thành 20 nhi®m vø sß môn, không nhæng nh§n ğßşc 1 cái bánh ú, mà còn nh§n ğßşc thêm 1#Y#{_INFOMSG%s}."
...
function x500501_OnSubmit_Necessary( sceneId, selfId, targetId, isHelpFinish )
	...
	local iQuarterTime = mod(iTime,100)	--ÉÏmµt ´Î·ÅÆúÈÎÎñtoÕ ğµ Ê±¼ä(¿Ì)
	...
	if iDayHuan==20 or iDayHuan==40 or iDayHuan==60  then
		...
		if curDayTime >= x500501_g_StartDayTime and curDayTime <= x500501_g_EndDayTime then
			...
			if canAdd1 == 0  then
				BeginEvent( sceneId )
					AddText( sceneId, "Cµt v§t ph¦m cüa các hÕ không còn ch± tr¯ng, l¥n sau lúc giao nhi®m vø xin hãy ğ¬ cµt ph¦m v§t ğü ch± tr¯ng." )
				EndEvent( sceneId )
...
--¸Ä³É²»¿ÉÄÜThöa mãntoÕ ğµ Ìõ¼şÀ´HuÖ böLinh Thúµ¤toÕ ğµ ½±Àø,by zhangqiang 2009.6.18
--		if GetLevel(sceneId, selfId) >= 50 and iDayHuan == 20 then --50c¤pÒÔÉÏ²Å ğÕt ğßşc,Ö»ÓĞµÚmµt cái20»·²Å¸ø,¶ø²»ĞúngÃ¿20»·¶¼¸ø
		if 0 > 1 then
			...
			if canAdd == 0  then
				...
				--¸ø×Ô¼º·¢ËÍ ği¬mµ½ÎïÆ·toÕ ğµ ÏûÏ¢....
				local strMsg = format("Các hÕ ğã ğÕt ğßşc #H#{_ITEM%d}#W.", x500501_g_LingShouDanId[randomIndex])
			...
			--[/tx42684]			
				...
				if random(100) <= 5  then
					...
					if canAdd == 0  then
						...
						--¸ø×Ô¼º·¢ËÍ ği¬mµ½ÎïÆ·toÕ ğµ ÏûÏ¢....
						strMsg = format("Các hÕ ğã ğÕt ğßşc #H#{_ITEM%d}#W.", x500501_g_LingShouDanId[randomIndex])
						Msg2Player(sceneId, selfId, strMsg, MSG2PLAYER_PARA)
...
--					-- ·¢ÊÀ½ç¹«¸æ
--					local transfer = GetItemTransfer(sceneId,selfId,0)
--					local str = format("#W#{_INFOUSR%s}#cffffcc hôm nay vì lúc c¯ g¡ng hoàn thành 20 l¥n nhi®m vø sß môn, do quá m®t möi ng°i dß¾i g¯c cây nghï ng½i, thì trên cây r½i xu¯ng 1 viên #W#{_INFOMSG%s}.",GetName(sceneId,selfId), transfer)
--					--¹«¸æTinh¼ò,È¥³ı´Ë¹«¸æ
--					--BroadMsgByChatPipe(sceneId, selfId, str, 4)
--					
...
--					-- ·¢ÊÀ½ç¹«¸æ
--					local transfer = GetItemTransfer(sceneId,selfId,0)
--					local str = format("#W#{_INFOUSR%s}#cffffcc hôm nay vì lúc c¯ g¡ng hoàn thành 20 l¥n nhi®m vø sß môn, do quá m®t möi ğã ng°i nghï dß¾i ğ¤t, t× trong túi r½i ra 1 viên #W#{_INFOMSG%s} l¤p lánh.",GetName(sceneId,selfId), transfer)
--					----¹«¸æTinh¼ò,È¥³ı´Ë¹«¸æ
--					--BroadMsgByChatPipe(sceneId, selfId, str, 4)
--					
--				end
				...
				--¸ø×Ô¼º·¢ËÍ ği¬mµ½ÎïÆ·toÕ ğµ ÏûÏ¢....
				local strMsg = format("Các hÕ ğã ğÕt ğßşc #c00FFFF#{_ITEM%d}#W.", x500501_g_ItemId[randomIndex])
				Msg2Player(sceneId, selfId, strMsg, MSG2PLAYER_PARA)
  ...
  -- ========================================================
	-- Ê¥µ®»î¶¯ Íê³ÉNhi®m vø sß môn ğÕt ğßşc·û½Ú
...
--	Msg2Player(  sceneId, selfId,"Äã ği¬mµ½ÁË#Y1#W ği¬mÃÅÅÉ¹±Ï×¶È." ,MSG2PLAYER_PARA )
...
--	Msg2Player(  sceneId, selfId,"Äã ği¬mµ½ÁË#Y2#W ği¬mÃÅÅÉ¹±Ï×¶È." ,MSG2PLAYER_PARA )
--else --mod(iDayHuan, 10) must return 0
--	SetHumanMenpaiPoint(sceneId, selfId, GetHumanMenpaiPoint(sceneId, selfId)+3	)
--	Msg2Player(  sceneId, selfId,"Äã ği¬mµ½ÁË#Y3#W ği¬mÃÅÅÉ¹±Ï×¶È." ,MSG2PLAYER_PARA )
--end

	--Êµ¼Ê½±Àø¹±Ï×¶È=(Íæ¼ÒµÈc¤p-»ù´¡µÈc¤p)* µÈc¤pÓ°Ïì²ÎÊı + »ù´¡¹±Ï×¶È
	...
	Msg2Player(  sceneId, selfId,"Các hÕ ğÕt ğßşc #Y" .. menpaiPoint .. "#Wği¬m ğµ c¯ng hiªn môn phái" ,MSG2PLAYER_PARA )
	...
	--//////////////////////////////////////////////////
	--ÓĞ10%toÕ ğµ ¼¸ÂÊ ğÕt ğßşcmµt cáiµÍc¤p±¦Ê¯ºÏ³É·û
	--AAA½ñÈÕÎªÊ¦ÃÅ×öÈÎÎñÓÂÍùÖ±Ç°,·Ü²»¹ËÉí,TÕi Íê³ÉµÚ20´Î
	--¶à±¶Ê±,ÖÕÒòÌ«ÀÛmµt õÓË¤µ¹TÕi Â·±ß,ÅÀÆğÀ´toÕ ğµ Ê±ºò,Á³ÉÏ
	--¾ÓÈ»Õ³×Åmµt ÕÅÖå°Í°ÍtoÕ ğµ [µÍc¤p±¦Ê¯ºÏ³É·û].
	--ID: 30900015
	--?	Íæ¼ÒµÈc¤p±ØĞë >=40c¤p
	...
	if playerLevel >= 40 and iDayHuan == 20 then
		...
		if ret <= 10 then
			...
			if canAdd > 0 then
				...
				local strformat	= "#W#{_INFOUSR%s}#cffffcc hôm nay vì lúc c¯ g¡ng hoàn thành 20 l¥n nhi®m vø sß môn, cu¯i cùng do quá m®t möi ğã ngã bên ğß¶ng, lúc ğÑng d§y, thì trên m£t b¸ dán 1 t¤m #W#{_INFOMSG%s} nhån nheo."
	...
	local MijiActive = 1
	local AwardID = 30505078    --ÌìÊé²Ğ¾ítoÕ ğµ ID
	local AwardCyc = 20         --½±Àø»·Êı( ×ö¶àÉÙ´ÎÈÎÎñ»á¸ømµt ´Î½±Àø )
	...
	if DayTime >= 7128 and DayTime <= 7150 then		-- 07.5.9 ~ 07.5.31
		if ( iDayHuan / cycle ) == floor( iDayHuan / cycle ) then
			...
			if AddRet > 0 then
				...
				local strText = "Vì túi ğñng ğ° cüa các hÕ không ğü, các hÕ m¤t mµt l¥n nh§n ğßşc #{_ITEM" .. skyrocket .. "} c½ hµi"
	...
	--	x500501_g_Exp = 2400*(Level+4) * 10 * l_Exp / 120						--µÈc¤p+»·Êıº¯Êı,ÊÜKinh nghi®mµ÷½Ú³£ÊıtoÕ ğµ Ó°Ïì
		--x500501_g_Money = 2400*(Level+4) * 10 * l_Money /120						--µÈc¤p+»·Êıº¯Êı,ÊÜKinh nghi®mµ÷½Ú³£ÊıtoÕ ğµ Ó°Ïì
	--else
	--	x500501_g_Exp = 2400*(Level+4) * mod(x500501_g_MissionRound,10) * l_Exp / 120						--µÈc¤p+»·Êıº¯Êı,ÊÜKinh nghi®mµ÷½Ú³£ÊıtoÕ ğµ Ó°Ïì
		--x500501_g_Money = 2400*(Level+4) * mod(x500501_g_MissionRound,10) * l_Money /120						--µÈc¤p+»·Êıº¯Êı,ÊÜKinh nghi®mµ÷½Ú³£ÊıtoÕ ğµ Ó°Ïì
	--end

	-- 1+µÈc¤pÓ°Ïì*µ±Ç°µÈc¤p/10 {µÈc¤pÓ°Ïì=0.035,Ó°ÏìÖ¸Êı=1}
	...
	if Reward_Append == 2 then
		if GetHuashanV(sceneId,selfId) == GetMenPai(sceneId,selfId) then
			...
			strHuashan = "#cff99ccVì b±n môn phái ğÕt ğßşc Lu§n Kiªm Hoa S½n hÕng nh¤t cüa kÏ này, nên có th¬ nh§n ğßşc thêm #Yph¥n thß·ng kinh nghi®m#W cüa sß môn." .. addExp .. "#cff99cc, ti«n#{_MONEY" .. addMoney .. "}#cff99cc."
			strHuashan2 = "#cff99cc nh§n ğßşc thêm #Yph¥n thß·ng kinh nghi®m#W cüa sß môn (Lu§n Kiªm Hoa S½n hÕng nh¤t) " .. addExp .. ", ti«n#Y#{_MONEY" .. addMoney .. "}#cff99cc."
		else
	...
	--	x500501_g_Exp = 2400*(Level+4) * l_Exp / 120 + x500501_g_Exp						--11~20»·ÈÎÎñ,Ã¿»·¶îÍâÔö¼Ómµt ¶¨Kinh nghi®mÔöÁ¿,½ğÇ®Ã»ÓĞÔöÁ¿
	--end
	...
	--ÏÔÊ¾¶Ô»°¿ò
	BeginEvent(sceneId)
		--Èç¹ûĞúngÍ¬ÃÅÏàÖúÍê³ÉtoÕ ğµ ....Ôò¼ÓÈëÏà¹Ø¶Ô»°....
		if isHelpFinish and isHelpFinish == 1 then
			AddText(sceneId, x500501_g_strHelpFinishedText)
		end
		AddText(sceneId,"  Làm r¤t khá, · ğây có "..x500501_g_Exp.." ği¬m tr¸ giá kinh nghi®m và #{_MONEY"..x500501_g_Money.."}, coi nhß t£ng thß·ng cho ngß½i")
		AddText(sceneId, strHuashan)
	EndEvent(sceneId)
	DispatchEventList(sceneId,selfId,targetId)

	Msg2Player(  sceneId, selfId, strHuashan2,MSG2PLAYER_PARA )
	--Msg2Player(  sceneId, selfId,"Äã ği¬mµ½ÁË#{_MONEY" .. tostring(x500501_g_Money) .. "}" ,MSG2PLAYER_PARA )
	--Msg2Player(  sceneId, selfId,"Äã ği¬mµ½ÁË" .. tostring(x500501_g_Exp) .. " ği¬mKinh nghi®mÖµ" ,MSG2PLAYER_PARA )

	--TÕi mµt ¶¨Ìõ¼şÏÂ¿ÉÒÔ ği¬mµ½¶îÍâtoÕ ğµ ½±Àø
	if Reward_Append == 2 and mod(iDayHuan, 5) == 0 and random(1000) <= 5 then
		...
		if canAdd > 0 then
			AddItemListToHuman(sceneId,selfId)
			szItemTransfer = GetItemTransfer(sceneId,selfId,0)
			local strText = format("Ngß½i ğã ğÕt ğßşc %s", ItemName)
			BeginEvent(sceneId)
				AddText(sceneId, strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			...
			local strformatList	= {
						"#W#{_INFOUSR%s}#cffffcc trong lúc làm nhi®m vø sß môn, vì quá v¤t vä, sß phø ğã thß·ng 1 #W#{_INFOMSG%s}. ",
						"#W#{_INFOUSR%s}#cffffcc v×a hoàn thành nhi®m vø sß môn bß¾c ra cØa thì b¸ v¤p phäi th«m cØa, phát hi®n 1 #W#{_INFOMSG%s}. ",
						"#cffffccVì #W#{_INFOUSR%s}#cffffcc trong lúc giao nhi®m vø sß môn, sß phù thì ğang n±i gi§n nên b¸ sß phø sÇn tay l¤y 1 #W#{_INFOMSG%s}#cffffcc ném trúng ğ¥u.",
						"#W#{_INFOUSR%s}#cffffcc trong lúc giao nhi®m vø sß môn, b²ng trên tr¶i r½i xu¯ng 1 #W#{_INFOMSG%s}#cffffcc trúng vào ğ¥u.",
						}
	...
	BeginEvent(sceneId)
		AddText(sceneId, "#GHôm nay#W ngß½i ğã hoàn thành xong " .. iDayHuan .. "#G vòng nhi®m vø sß môn#W");
	EndEvent(sceneId)
	DispatchMissionTips(sceneId,selfId)

	--Nhi®m vø sß mônÍ³¼Æ
	LuaFnAuditShiMenRenWu(sceneId, selfId, x500501_g_MissionRound, GetLevel(sceneId, selfId))
	LuaFnAuditShiMenRenWu_Day(sceneId, selfId, iDayHuan, GetLevel(sceneId, selfId))           --µ±ÌìNhi®m vø sß mônÍ³¼Æ
...
function x500501_CheckAccept_Necessary( sceneId, selfId )
	...
	local iTime = GetMissionData(sceneId,selfId,MD_SHIMEN_DAYTIME)
	local iDayTime = floor(iTime/100)	--ÉÏmµt ´Î·ÅÆúÈÎÎñtoÕ ğµ Ê±¼ä(ÌìÊı)
	local iQuarterTime = mod(iTime,100)	--ÉÏmµt ´Î·ÅÆúÈÎÎñtoÕ ğµ Ê±¼ä(mµt ¿ÌÖÖ)
	--local iDayHuan = floor(iDayCount/100000) --µ±ÌìÄÚÍê³ÉtoÕ ğµ S¯ l¥n nhi®m vø 
	local iDayHuan = iDayCount --µ±ÌìÄÚÍê³ÉtoÕ ğµ S¯ l¥n nhi®m vø 
	...
	if iDayTime == CurDaytime then
		--Èç¹ûµ±Ìì×ö¸ÃS¯ l¥n nhi®m vø ´ïµ½60Ôò²»ÔÊĞíÔÙ½Ó
		if iDayHuan >= 60 then
			return -2;
		end
...
function x500501_Abandon_Necessary( sceneId, selfId )
	...
	if menpaiPoint == 1 then
		...
		Msg2Player(  sceneId, selfId,"Ngß½i ğã tiêu hao #Y1 #Wği¬m c¯ng hiªn môn phái" ,MSG2PLAYER_PARA )
	elseif menpaiPoint >= 2 then
		SetHumanMenpaiPoint(sceneId, selfId, menpaiPoint-2 )
		Msg2Player(  sceneId, selfId,"Ngß½i ğã tiêu hao #Y2 #Wği¬m c¯ng hiªn môn phái" ,MSG2PLAYER_PARA )
	end
end

--**********************************
--Íæ¼ÒÌá½»toÕ ğµ ÆÕÍ¨ÎïÆ·
--**********************************
...
function x500501_OnMissionCheck_NecessaryEx( sceneId, selfId, index1, index2, index3, Needindex )
	...
	for i = 1, getn( indices ) do
		if indices[i] and indices[i] >=0 and indices[i] < 80 then	-- ºÏ·¨toÕ ğµ ±³°ü¸ñ×Ó
  ...
  if DayHuan ~=20 and DayHuan ~=40 and DayHuan ~=60  then
    ...
    local strtext = ""
	  ...
	  if menpaiId == MP_SHAOLIN then
	     strtext = "Các hÕ thu ğßşc 1 t¤m Thi®p chúc ngày l­ phái Thiªu Lâm."
	     hekaid = x500501_HeKaId[1]
	  elseif menpaiId == MP_MINGJIAO then
	     strtext = "Các hÕ thu ğßşc 1 t¤m Thi®p chúc ngày l­ phái Minh Giáo."
	     hekaid = x500501_HeKaId[2]
	  elseif menpaiId == MP_GAIBANG then
	     strtext = "Các hÕ thu ğßşc 1 t¤m Thi®p chúc ngày l­ phái Cái Bang."
	     hekaid = x500501_HeKaId[3]
	  elseif menpaiId == MP_WUDANG then
	     strtext = "Các hÕ thu ğßşc 1 t¤m Thi®p chúc ngày l­ phái Võ Ğang."
	     hekaid = x500501_HeKaId[4]
	  elseif menpaiId == MP_EMEI then
	     strtext = "Các hÕ thu ğßşc 1 t¤m Thi®p chúc ngày l­ phái Nga My."
	     hekaid = x500501_HeKaId[5]
	  elseif menpaiId == MP_XINGSU then
	     strtext = "Các hÕ thu ğßşc 1 t¤m Thi®p chúc ngày l­ phái Tinh Túc."
	     hekaid = x500501_HeKaId[6]
	  elseif menpaiId == MP_DALI then
	     strtext = "Các hÕ thu ğßşc 1 t¤m Thi®p chúc ngày l­ phái Thiên Long."
	     hekaid = x500501_HeKaId[7]
	  elseif menpaiId == MP_TIANSHAN then
	     strtext = "Các hÕ thu ğßşc 1 t¤m Thi®p chúc ngày l­ phái Thiên S½n."
	     hekaid = x500501_HeKaId[8]
	  elseif menpaiId == MP_XIAOYAO then
	     strtext = "Các hÕ thu ğßşc 1 t¤m Thi®p chúc ngày l­ phái Tiêu Dao."
