...
x401001_g_CopySceneType = FUBEN_ZHENGLONG;	--¸±±¾ÀàÐÍ,¶¨ÒåTÕi ScriptGlobal.luaÀïÃæ
x401001_g_limitMembers = 1;					--¿ÉÒÔ½ø¸±±¾toÕ ðµ ×îÐ¡¶ÓÎéÈËÊý
x401001_g_tickDiffTime = 1;					--»Øµ÷½Å±¾toÕ ðµ Ê±ÖÓÊ±¼ä(µ¥Î»:  giây/´Î)
x401001_g_closeTickCount = 30;				--¸±±¾¹Ø±ÕÇ°µ¹¼ÆÊ±(µ¥Î»: ´Î)
x401001_g_startTickCount = 30;				--¿ªÊ¼³ö¹ÖtoÕ ðµ Ê±¼ä(µ¥Î»: ´Î)
x401001_g_tipsIntervalTickCount = 5;		--ÌáÊ¾ÐÅÏ¢toÕ ðµ ¼ä¸ôÊ±¼ä

x401001_g_NoUserTime = 300;					--¸±±¾ÖÐÃ»ÓÐÈËºó¿ÉÒÔ¼ÌÐø±£´ætoÕ ðµ Ê±¼ä(µ¥Î»:  giây)
x401001_g_Fuben_X = 40;						--½øÈë¸±±¾toÕ ðµ Î»ÖÃX
x401001_g_Fuben_Z = 40;						--½øÈë¸±±¾toÕ ðµ Î»ÖÃZ
x401001_g_Relive_X = 45;					--¸±±¾¸´»îtoÕ ðµ Î»ÖÃX
x401001_g_Relive_Z = 40;					--¸±±¾¸´»îtoÕ ðµ Î»ÖÃZ
x401001_g_Back_X_Ly = 268;						--LÕc Dß½ngÔ´³¡¾°Î»ÖÃX
x401001_g_Back_Z_Ly = 87;						--LÕc Dß½ngÔ´³¡¾°Î»ÖÃZ
x401001_g_Back_X_Sz = 175;						--Tô ChâuÔ´³¡¾°Î»ÖÃX
x401001_g_Back_Z_Sz = 145;						--Tô ChâuÔ´³¡¾°Î»ÖÃZ
x401001_g_Back_X_Dl = 275;						--ÐÕi LýÔ´³¡¾°Î»ÖÃX
x401001_g_Back_Z_Dl = 95;						--ÐÕi LýÔ´³¡¾°Î»ÖÃZ

x401001_g_aToBInterval = 7 * 1000;			--ÓÉ²»¿É¹¥»÷×´Ì¬±ä³É¿É¹¥»÷×´Ì¬toÕ ðµ Ê±¼ä(µ¥Î»: ºÁ giây)
x401001_g_bToCInterval = 60 * 1000;			--ÓÉ¿É¹¥»÷×´Ì¬±ä³ÉÈËÐÎ¹ÖÎïtoÕ ðµ Ê±¼ä(µ¥Î»: ºÁ giây)
...
--ÆåÆ×Êý¾Ý
x401001_g_mStepPosListCount = 1;	--¿ÉÒÔÓÃÓÚÑ¡ÔñtoÕ ðµ ÆåÆ×ÊýÁ¿
x401001_g_mStepPosListSize = 201;	--Ã¿cáiÆåÆ×ÓµÓÐtoÕ ðµ ²½Êý
...
--x401001_CheckAndEnterSceneËùÓÃµ½toÕ ðµ ²ÎÊý
...
--**********************************
--½øÈë¸±±¾toÕ ðµ ÅÐ¶ÏºÍÖ´ÐÐ
--**********************************
function x401001_CheckAndEnterScene(sceneId, selfId, targetId, eventId)
	local strOutmsg = "#{QJXXS_081211_01}";  --"ÏÖTÕi ²»ÐúngÆå¾Ö»î¶¯¿ªÊ¼toÕ ðµ Ê±¼ä,µÈ»î¶¯¿ªÊ¼Ö®ºóÔÙÀ´ÕÒÎÒ.";
	...
	strOutmsg = "Tham gia Trân Long KÏ Cuµc yêu c¥u ðµi ngû phäi có "..x401001_g_limitMembers.." thành viên tr· lên.";
	...
	--bug27726,2007.11.15 by alan,ÒòÎªsceneMemberCount²»mµt ¶¨×¼È·,¸ÄÓÃ¸½½üÐµi viên ÊýÓë¶ÓÎéÈËÊýtoÕ ðµ ±È½ÏÅÐ¶Ï
	--²¢²»ÔÙÌáÊ¾¾ßÌå²»TÕi ¸½½ütoÕ ðµ Ðµi viên Ãû³Æ
	
	local nearMemberCount = GetNearTeamCount(sceneId, selfId);
	strOutmsg = "Thành viên trong ðµi ngû cüa các hÕ không · g¥n. Hãy §p hþp ð¥y ðü tÕi ðây r°i tìm ta ð¬ tham gia hoÕt ðµng.";
	...
	-- Í³¼ÆChßa ðüµÈc¤pÒªÇótoÕ ðµ Ðµi viên ÊýÁ¿
	local nLowLvlCnt = 0
	...
	--strOutmsg = "ÄúÓÐÐµi viên ²»TÕi ¸½½ü,Çë¼¯ºÏºóÔÙÕÒÎÒ½øÈë»î¶¯.";
	...
	local checkCurDayMsg = "Trong ðµi (";
	...
	elseif eventId == x401001_defaultEvent then
		BeginEvent(sceneId)	
			AddText(sceneId, "#{function_help_063}#r  Nªu ngß½i có th¬ th¤y chæ này, chÑng minh ngß½i ðã ðªn cänh gi¾i không ti«n khoáng h§u.");
		EndEvent(sceneId)
...
function x401001_MakeCopyScene(sceneId, selfId, nearmembercount)
	...
	LuaFnSetSceneLoad_Map(sceneId, "zhenlong.nav");										--µØÍ¼Ðúng±ØÐëÑ¡È¡toÕ ðµ ,¶øÇÒ±ØÐëTÕi Config/SceneInfo.iniÀïÅäÖÃºÃ
	...
	LuaFnSetCopySceneData_Param(sceneId, 0, x401001_g_CopySceneType);					--ÉèÖÃ¸±±¾Êý¾Ý,ÕâÀï½«0ºÅË÷ÒýtoÕ ðµ Êý¾ÝÉèÖÃÎª999,ÓÃÓÚ±íÊ¾¸±±¾ºÅ999(Êý×Ö×Ô¶¨Òå)
	...
	LuaFnSetCopySceneData_Param(sceneId, 8, random(1, x401001_g_mStepPosListCount));	--Ñ¡ÓÃtoÕ ðµ ÆåÆ×
	LuaFnSetCopySceneData_Param(sceneId, 9, 0);											--ÒÑ¾­ÏÂÁËtoÕ ðµ ²½Êý
	LuaFnSetCopySceneData_Param(sceneId, 11, 0);										--ÒÑ¾­É±ÁË¶àÁËcái¹ÖÁË
	LuaFnSetCopySceneData_Param(sceneId, 12, 0);										--ÉÏ´Î³ö¹ÖÊ±¼ä
	LuaFnSetCopySceneData_Param(sceneId, 13, 0);										--ÆÕÍ¨Ä£Ê½ÓëÐÂÊÖÄ£Ê½Ñ¡Ôñ	0:ÆÕÍ¨	1:´ømµt cáiÐÂÈË	2:	´øÁ½cáiÒÔÉÏtoÕ ðµ ÐÂÈË
	
	-- ¼ì²âÐúng·ñ´øÐÂÊÖÄ£Ê½
	if x401001_OnXinShouCount( sceneId, selfId ) >= 2 then
		LuaFnSetCopySceneData_Param(sceneId, 13, 2);										--Á½cáiÒÔÉÏtoÕ ðµ ÐÂÊÖ
	elseif x401001_OnXinShouCount( sceneId, selfId ) > 0 then
		LuaFnSetCopySceneData_Param(sceneId, 13, 1);										--Ö»ÓÐmµt cáiÐÂÊÖ
	end
  ...
  local PlayerMaxLevel = GetHumanMaxLevelLimit()
	...
	if iniLevel/10 <= x401001_g_MoTypeCount then
		LuaFnSetCopySceneData_Param(sceneId, 10, iniLevel/10);							--¼ÇÂ¼Ëù²úÉútoÕ ðµ ¹ÖÎï×é
	else
		LuaFnSetCopySceneData_Param(sceneId, 10, x401001_g_MoTypeCount);				--¼ÇÂ¼Ëù²úÉútoÕ ðµ ¹ÖÎï×é
	end
...
--***********************************
-- ´´½¨10cáiÄêÊÞ add by cuiyinjie 2008-11-6
--***********************************
--function x401001_CreateNianshou(sceneId)
--	local iNianshouCreateFlagIndex = 20; --×î´ó32cái²ÎÊý
--	-- ÅÐ¶ÏÒÑ¾­´´½¨¾Í²»ÔÙ´´½¨ÁË
...
--		local dis = LuaFnGetCopySceneData_Param(sceneId, CopyScene_LevelGap);	--c¤p±ð²î,CopyScene_LevelGap TÕi  scene.lua ÖÐ¸³Öµ	
...
--	x401001_CreateNianshou(sceneId); -- add by cuiyinjie 2008-11-6
	...
	if leaveFlag == 1 then --C¥n Àë¿ª
		...
		if curTickCount >= x401001_g_startTickCount then	--Ê±¼äµ½,¿ªÊ¼³ö¹Ö
			local mgroup = LuaFnGetCopySceneData_Param(sceneId, 10);	--¼ÇÂ¼Ëù²úÉútoÕ ðµ ¹ÖÎï×é
			local xinshoucnt = 0
			...
			-- ¸ù¾ÝÆÕÍ¨ºÍÐÂÈËÄ£Ê½Ñ¡Ôñ²»Í¬toÕ ðµ monsterÀàÐÍ
			...
			for i = 0, monsterCount - 1 do
				...
				if LuaFnIsCharacterLiving(sceneId, monsterObjId) > 0 then	--»îtoÕ ðµ ¹ÖÎï
					...
					if monstertype == Black_A[mgroup] then
						if nowTime >= mcreatetime + x401001_g_aToBInterval then --C¥n ½«¹ÖÎï±ä³ÉB×´Ì¬ÁË
					...
					elseif monstertype == Black_B[mgroup] then
						if nowTime >= mcreatetime + x401001_g_aToBInterval + x401001_g_bToCInterval then --C¥n ½«¹ÖÎï±ä³ÉC×´Ì¬ÁË
							LuaFnDeleteMonster(sceneId, monsterObjId);
						...
						if newObjId and newObjId > -1 then
							...
							if isBoss == 1 then
								...
								if szName == "Vi­n C± KÏ H°n"   then
									SetCharacterTitle(sceneId, newObjId, "Kì Vß½ng")
								end
...
function x401001_OnDie(sceneId, selfId, killerId)						-- ³¡¾°ID, ±»É±toÕ ðµ ObjId, É±ÊÖObjId
	...
	-- ´øÐÂÊÖÄ£Ê½ÏÂÉÆ¶ñÖµºÍ¶îÍâKinh nghi®mtoÕ ðµ ¸øÓè
	-- ¼ì²âÐúng·ñThöa mãn´øÐÂÊÖÄ£Ê½
	if playerID ~= nil and playerID ~= -1 and x401001_OnXinShouCount( sceneId, playerID ) > 0 then
		...
		if memlevel >= x401001_g_Highlvl_Limit then
			...
			if teamLeaderFlag == 1 then
				...
				LuaFnSetHumanGoodBadValue(sceneId, playerID, memgoodbad+x401001_g_High_GoodBadParam)		-- É±ÊÖÐúng¶Ó³¤toÕ ðµ »°¶à¸øÐ©ÉÆ¶ñ
				nStr = format( "#{QJ_20080522_04}%d#{QJ_20080522_05}", x401001_g_High_GoodBadParam )
			else
				--PrintStr("Low_GoodBadParam")
				LuaFnSetHumanGoodBadValue(sceneId, playerID, memgoodbad+x401001_g_Low_GoodBadParam)			-- É±ÊÖÐúngÐµi viên toÕ ðµ »°ÉÙ¸øÐ©ÉÆ¶ñ
		...
		for	k = 0, memcount - 1 do
			...
			if memlvl >= x401001_g_Lowlvl_Limit.from and memlvl <= x401001_g_Lowlvl_Limit.to then
				...
				--local nExpAtn = 0 --Kinh nghi®mË¥¼õÒÑ¾­HuÖ bö,by hukai
				local PlayerExpList = {}
				local	ExpParam = x401001_g_XinShou_Exp_Mod01										-- Kinh nghi®mÏµÊý¼Ó³É,Æ½Ê±¸øtoÕ ðµ ÉÙ
				--nExpAtn = GetExpAttenuationInfo( sceneId, memberId, levelcur )	-- ¸ù¾ÝExpAttenuation.txt ðÕt ðßþcKinh nghi®mË¥¼õÏµÊý,ÒÑ¾­HuÖ bö,by hukai
				--PrintStr("nExpAtn ="..nExpAtn)
				for n, ExpListInfo in x401001_g_XinShou_ExpAtu do
					if ( objType and objType >= ExpListInfo.mBengin01 and objType <= ExpListInfo.mEnd01)						-- ¸ù¾ÝmonsterÀàÐÍ,¸ù¾ÝÃ¿cáiÍæ¼ÒtoÕ ðµ µÈc¤p¸øÓè²»Í¬toÕ ðµ ¶îÍâKinh nghi®m
				...
				if DEIsLock(sceneId,memberId) <= 0 then								-- Íæ¼ÒÐúng·ñLînh µ½Ë«±¶/ËÄ±¶Kinh nghi®m½±Àø,²»°üÀ¨¶³½á×´Ì¬
					if nFreeHave > 0 or nMoneyHave > 0 then
						--PrintStr("Exp_Mod02")
						ExpParam = x401001_g_XinShou_Exp_Mod02						-- Î´¶³½átoÕ ðµ Ë«±¶/ËÄ±¶Kinh nghi®mKinh nghi®mÏµÊý¼Ó³É¸ß
					end
				end				

				-- ¸ù¾Ý×é¶ÓÈËÊýKinh nghi®mË¥¼õ
				playerExp01 = playerExp01*( ( 1+( memcount-1 )/x401001_g_ExpTeamPre )/memcount )
				--local ExpSum = (1-nExpAtn)*playerExp01*ExpParam				-- ¸ù¾Ý¹«Ê½¸øÓèÍæ¼ÒKinh nghi®m.Kinh nghi®mË¥¼õÒÑ¾­HuÖ bö by hukai
				local ExpSum = playerExp01*ExpParam				-- ¸ù¾Ý¹«Ê½¸øÓèÍæ¼ÒKinh nghi®m
...
--	    end
		...
		local membercount = LuaFnGetCopyScene_HumanCount(sceneId);
		local memId
		local strText = format("Ngß½i ðã hÕ sát %d/%d quân c¶", curKillCount, x401001_g_mStepPosListSize - 1);
		for	i = 0, membercount - 1 do
			memId = LuaFnGetCopyScene_HumanObjId(sceneId, i);
			if LuaFnIsObjValid( sceneId, memId ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, memId ) == 1 then
				x401001_NotifyFailTips(sceneId, memId, strText);
...
--  ðÕt ðßþc´øÐÂÊÖÄ£Ê½ÏÂÐÂÊÖtoÕ ðµ ÊýÁ¿
