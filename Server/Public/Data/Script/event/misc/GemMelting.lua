...
function x800118_OnGemMelting( sceneId, selfId, GemItemPos1, GemItemPos2, GemItemPos3, NeedItemPos )
	...
	--Ðúng·ñTÕi °²È«Ê±¼äÄÚ....Cái này ÅÐ¶Ïº¯ÊýÀïÃæ×Ô¼ºÓÐÌáÊ¾ÐÅÏ¢....
	...
	--¼ì²âÐúng·ñÐúngÓÐÍ¬ÑùtoÕ ðµ ±¦Ê¯....
	if GemItemID1 ~= GemItemID2 or GemItemID1 ~= GemItemID3 then
		x800118_NotifyTip( sceneId, selfId, "±¦Ê¯ÀàÐÍ²»mµt ÖÂ,±¦Ê¯ÈÛÁ¶th¤t bÕi." )
		return
	end
	...
	if  -1 == ProductID then
		x800118_NotifyTip( sceneId, selfId, "¸Ã±¦Ê¯ÎÞ·¨ÈÛÁ¶,±¦Ê¯ÈÛÁ¶th¤t bÕi." )
		return
	end

	--¼ì²âÐúng·ñÓÐËùÐèÎïÆ·....
	if NeedID ~= NeedItemID then
		x800118_NotifyTip( sceneId, selfId, "ÈÛÁ¶ËùÐèÎïÆ·´íÎó,±¦Ê¯ÈÛÁ¶th¤t bÕi." )
		return
	end
	...
	if PlayerMoney < NeedMoney then
		x800118_NotifyTip( sceneId, selfId, "Không ðü ngân lßþng.,±¦Ê¯ÈÛÁ¶th¤t bÕi." )
		return
	end
	...
	if ret ~= 1 then
		x800118_NotifyTip( sceneId, selfId, "±¦Ê¯ÎÞ·¨Ê¹ÓÃ,±¦Ê¯ÈÛÁ¶th¤t bÕi." )
		return
	end
	...
	if ret ~= 1 then
		x800118_NotifyTip( sceneId, selfId, "ËùÐèÎïÆ·ÎÞ·¨Ê¹ÓÃ,±¦Ê¯ÈÛÁ¶th¤t bÕi." )
		return
	end


	-- ±ØÐë±£Ö¤¿Û³ýÎïÆ·È«²¿³É¹¦,ÈÎºÎmµt ´Îth¤t bÕi¶¼µ¼ÖÂÎÞ·¨ºÏ³É±¦Ê¯,²¢ÇÒÒÑ¾­¿Û³ýtoÕ ðµ ±¦Ê¯²»¹é»¹,added by dun.liu 2009.2.5
	if LuaFnEraseItem( sceneId, selfId, GemItemPos1 ) == 0 then
		x800118_NotifyTip( sceneId, selfId, "¿Û³ýÎïÆ·th¤t bÕi,ÎÞ·¨ÈÛÁ¶±¦Ê¯." )
		return
	end
	...
	--¸øÍæ¼ÒÈÛÁ¶ºótoÕ ðµ ±¦Ê¯....²»ÓÃ¼ì²â±³°üÐúng·ñÓÐµØ·½....Ã»µØ·½Ç°±ßÒ²del³öµØ·½ÁË....
	local BagIndex = TryRecieveItem( sceneId, selfId, ProductID, QUALITY_MUST_BE_CHANGE )
	if BagIndex == -1 then
		x800118_NotifyTip( sceneId, selfId, "±³°üÒÑÂú,±¦Ê¯ÈÛÁ¶th¤t bÕi." )
		return
	end
	if bNeedBind == 1 then
	...
	--ÐÑÄ¿ÌáÊ¾....
	x800118_NotifyTip( sceneId, selfId, "3¿Å(#{_ITEM"..GemItemID1.."})±»³É¹¦ÈÛÁ¶³Émµt ¿Å(#{_ITEM"..ProductID.."})" )

	--¹«¸æ....
	if LuaFnGetItemQuality(ProductID) >= 3 then
		local MeltingNPCTbl =
		{
			[0]   = "LÕc Dß½ng(178,185)Åí»³Óñ",
			[420] = "Thúc Hà C± Tr¤n(134,84)¾£Ç¶Êµ",
			[186] = "Lâu Lan(74,161)¿ËÀïÄ¾",
		}
