--Giám Ð¸nh Tß Ch¤t Trang B¸
--½Å±¾ºÅ
x809261_g_ScriptId = 809261

--Giám Ð¸nh Tß Ch¤t Trang B¸UI 1001
...
function x809261_FinishReAdjust( sceneId, selfId, nEquItemIndex )
	...
	if itemCount < 1 and itemCount2 < 1 then
		BeginEvent(sceneId)
			AddText(sceneId,"Các hÕ thiªu Kim Cß½ng Sa.");
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
		return
	end
	
	-- ¼ì²â×°±¸×ÊÖÊtoÕ ðµ ¼ø¶¨Çé¿ö....
	...
	--¿Û³ý¹æÔòÐúng: °ó¶¨ÓÅÏÈ,È»ºóÐúng½ð¸ÕÉ°ÓÅÏÈ.Ò²¾Í¿Û³ý´ÎÐòÐúng°ó¶¨½ð¸ÕÉ°,°ó¶¨½ð¸Õï±,½ð¸ÕÉ°,½ð¸Õï±
	--ÕÒ°ó¶¨½ð¸ÕÉ°
	...
	if ShaPos ~= -1 then
		...
		if ret ~= 1 then
			BeginEvent(sceneId)
				AddText(sceneId,"Kim Cß½ng Sa không th¬ sØ døng.");
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			return
		end
	elseif ShaPos2 ~= -1 then
		...
		local r, t = LuaFnEraseItemTimes( sceneId, selfId, ShaPos2, 11 )
			...
			BeginEvent(sceneId)
			local	szMsg				= format( "S¯ l¥n sØ døng còn Õi: %d/%d", tonumber(t), tonumber(11) );
	...
	if ret == 1 then
		...
		BeginEvent(sceneId)
			AddText(sceneId,"Giá ð¸nh lÕi tß ch¤t trang b¸ thành công");
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
		return
...
function x809261_ReAdjustNotify( sceneId, selfId, ret, ShaPos, ShaPos2, ShaInfo, nEquItemIndex )
	...
	--Ö»ÓÐÌØ¶¨¼¸ÀàtoÕ ðµ ×°±¸Ï´³öÁË250ÒÔÉÏtoÕ ðµ ×ÊÖÊÊ±....ÔÊÐí·¢Ï´³ö³¬c¤p×°±¸toÕ ðµ ¹«¸æ....
	--1Ã±×Ó 2ÒÂ·þ 3ÊÖÌ× 4Ð¬ 15»¤¼ç
	-- Ìí¼Ó 0ÎäÆ÷¡¢6,11½äÖ¸(2cái)¡¢12,13»¤·û(2cái)
	if ret == 3 then
	...
	--Èç¹û×ÊÖÊÏ´µ½ÁË250ÒÔÉÏ¾Í»á·¢Ï´³ö³¬c¤p×°±¸toÕ ðµ ¹«¸æ....
	if ret == 3 then
	...
	--Èç¹û×ÊÖÊÃ»ÓÐÏ´µ½ÁË250ÒÔÉÏ....µ«Ò²ÒÑ¾­´ïµ½ÓÅÐã....¾Í»á·¢Ï´³öÆÕÍ¨×°±¸toÕ ðµ ¹«¸æ....
	if ret == 2 then
