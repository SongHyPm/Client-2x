...
-- ÏâÇ¶²»Í¬µÈc¤p±¦Ê¯toÕ ðµ ½ðÇ®ÏûºÄ±í
...
function x701614_GemEmbed( sceneId, selfId, GemBagIndex, EquipBagIndex, MatBagIndex1, MatBagIndex2 )
	...
	-- ñÒÉÙÎ¢,2008.7.1.ÖØÂ¥½ä¡¢ÖØÂ¥ÓñtoÕ ðµ »úÖÆÐÞ¸Ä: 1¡¢¿ÉÒÔ´ò¿×£»2¡¢¿ÉÒÔÏâÇ¶±¦Ê¯,µ«Ö»ÄÜÏâÇ¶²»°ó¶¨toÕ ðµ ±¦Ê¯
	if(GemIsBind == 1) then
  ...
  if MatBagIndex1 ~= -1 then
	...
	-- ÅÐ¶Ï±¦Ê¯ºÏ·¨ÐÔ
	if LuaFnIsItemAvailable( sceneId, selfId, GemBagIndex ) < 1 then	-- Ê¹ÓÃÓÐÎÊÌâtoÕ ðµ ÎïÆ·ÔòÍË³öÁ÷³Ì
		return OR_STUFF_LACK
	else
	...
	or equipType == -1 then												-- Ê¹ÓÃÓÐÎÊÌâtoÕ ðµ ÎïÆ·ÔòÍË³öÁ÷³Ì
		return OR_STUFF_LACK
	else
	...
	-- ÅÐ¶ÏºÍ×°±¸ÉÏtoÕ ðµ ÏÖÓÐ±¦Ê¯Ðúng·ñÓÐ³åÍ»
	...
	if odds >= randRet then
		...
		if ret and ret == 0 then
			...
			--¹«¸æTinh¼ò,Ð¡ÓÚ3c¤ptoÕ ðµ ±¦Ê¯ÏâÇ¶²»·¢¹«¸æÏûÏ¢
			if (gemQual >= 3) then
	...
	-- Èç¹ûth¤t bÕi
		...
		if gemQual - qualDec > 0 then
			...
			x701614_MsgBox( sceneId, selfId, "#YTh§t không may, khäm nÕm th¤t bÕi bäo thÕch "..qualDec.." xu¯ng mµt b§t." )
		else
...
function x701614_GemEmbed_Four( sceneId, selfId, GemBagIndex, EquipBagIndex, MatBagIndex1, MatBagIndex2 )
  ...
  if MatBagIndex1 ~= -1 then
	...
	if LuaFnIsItemLocked( sceneId, selfId, EquipBagIndex ) ~= 0	or equipType == -1 then												-- Ê¹ÓÃÓÐÎÊÌâtoÕ ðµ ÎïÆ·ÔòÍË³öÁ÷³Ì
		return OR_STUFF_LACK
	else
		equipIdx = LuaFnGetItemTableIndexByIndex( sceneId, selfId, EquipBagIndex )
	end
