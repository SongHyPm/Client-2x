-- ¾ÛÏÍ×¯Ö® LÕc Dß½ng³¡¾°»¤ËÍ,ÖØ ği¬m²¿·ÖÅ¶
-- ´¥·¢Ìõ¼ş,Hoàn t¤t nhi®m vø21,È»ºó½øÈëLÕc Dß½ngtoÕ ğµ ÊÂ¼şÇøÓò
...
--ÈÎÎñÈë¿Úº¯Êı
--**********************************
function x200022_OnDefaultEvent( sceneId, selfId, targetId, missionIndex )	-- ği¬m»÷¸ÃÈÎÎñºóÖ´ĞĞ´Ë½Å±¾
...
function x200022_OnAccept( sceneId, selfId, targetId )
	...
	-- ÕâÀïtoÕ ğµ »¤ËÍC¥n Æô¶¯³¡¾°toÕ ğµ ¼ÆÊ±Æ÷,²»ÄÜÊ¹ÓÃÈÎÎñ¼ÆÊ±Æ÷
	if x200022_g_Step == 0   then
		...
		-- Cái này ³¡¾°¼ÆÊ±Æ÷,Ó¦¸ÃĞúngmµt Ö±±»Cái này »î¶¯Õ¼ÓÃtoÕ ğµ ,ËùÒÔ,Ö»ÒªCái này Öµ²»µÈÓÚ-1,¾Í²»ÒªÔÙÉĞúngë
	...
	if x200022_g_Step == 2   then
		-- °ÑCái này Íæ¼Ò¼Óµ½»¤ËÍÁĞ±íÖĞÈ¥
		if x200022_g_PlayerNum >= 6   then
			-- ±¾´Î²Î¼Ó»¤ËÍtoÕ ğµ ÈËÊıÂúÁË,²»ÄÜÔÙ£«Äã
...
function x200022_OnSceneTimer( sceneId, selfId )
	...
	-- ¼ì²âÖ»ÒªKi«u PhongºÍ°¢ÖéÖĞtoÕ ğµ ÈÎºÎmµt cáiÈË²»TÕi ³¡¾°ÖĞÁË,¾ÍÇå¿ÕËùÓĞtoÕ ğµ È«¾Ö±äÁ¿
	...
	if x200022_g_Step == 1   then
		-- ¸øĞ¡¶ÓtoÕ ğµ ËùÓĞÍæ¼Ò·¢ËÍmµt Æğ»¤ËÍtoÕ ğµ ÑûÇë
...
--		PrintNum(x200022_g_PlayerNum)
	...
	elseif x200022_g_Step == 2   then
		-- µÈ´ı5 giây,Cái này Ê±¼äĞúngµÈ´ı±ğtoÕ ğµ Íæ¼Òmµt Æğ½øĞĞCái này »¤ËÍtoÕ ğµ Ê±¼ä
	...
	elseif x200022_g_Step == 3   then
		...
		-- ÉĞúngëmµt cáiÑ²Âß×é,¿ªÊ¼×ß
		--local groupId = AllocateMonsterGroup(sceneId)

		-- Ô­À´ĞúngÓÃtoÕ ğµ ĞúngÉĞúngëmµt cáiĞÂtoÕ ğµ Ñ²Âß×é,ÏÖTÕi Ê¹ÓÃmµt cái¹Ì¶¨toÕ ğµ Ñ²Âß×éÀ´Íê³É
		-- TÕi b¡t ğ¥uCái này ÈºÌå»¤ËÍÇ°,ÏÖÇå¿Õmµt ÏÂCái này ×é
	...
	elseif x200022_g_Step == 4   then
		...
		-- ¼ÆÊ±Æ÷Ã¿´Î¼ì²âÍæ¼ÒĞúng²»ĞúngÓĞÈÎÎñth¤t bÕitoÕ ğµ Çé¿ö³öÏÖ,
		for i=1, x200022_g_PlayerNum  do
		...
		-- TÕi ÕâÀïÌí¼ÓLÕc Dß½ng¾ÓÃñtoÕ ğµ ÅİÅİ,Ôö¼ÓÆø·Õ
		
		
		-- ¼ì²â»¤ËÍtoÕ ğµ Íê³ÉÌõ¼ş,Ki«u Phong ×ßµ½ÁËÄ¿±ê ği¬m
		local nMonsterNum = GetMonsterCount(sceneId)
		local jj = 0
		local bHaveMonster = 0
		for jj=0, nMonsterNum-1 do
			...
			if GetName(sceneId, nMonsterId)  == "Ki«u Phong"  then
				...
				if distance<=2   then
					...
					for i=1, x200022_g_PlayerNum   do
						if x200022_g_PlayerId[i]~=-1   then
							local misIndex = GetMissionIndexByID(sceneId, x200022_g_PlayerId[i], x200022_g_MissionId)-- ği¬mµ½ÈÎÎñTÕi 20cáiÈÎÎñÖĞtoÕ ğµ ĞòÁĞºÅ
...
function x200022_CheckSubmit( sceneId, selfId )
	local misIndex = GetMissionIndexByID(sceneId, selfId, x200022_g_MissionId)-- ği¬mµ½ÈÎÎñTÕi 20cáiÈÎÎñÖĞtoÕ ğµ ĞòÁĞºÅ
...
function x200022_OnSubmit( sceneId, selfId, targetId, selectRadioId, missionIndex )
	...
	Msg2Player(  sceneId, selfId,"#YHoàn t¤t nhi®m vø#W: Dù vÕn ngß¶i, ta vçn cÑ tiªn",MSG2PLAYER_PARA )
