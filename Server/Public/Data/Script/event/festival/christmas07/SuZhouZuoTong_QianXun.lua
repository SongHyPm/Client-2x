...
function x229024_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if key == x229024_g_QianXunMission_IDX then
		BeginEvent(sceneId)
			...
			AddNumText( sceneId, x229024_g_ScriptId, "·òÆŞLînh ÈÎÎñ", 6, x229024_g_FuQiMission_IDX )		
			AddNumText( sceneId, x229024_g_ScriptId, "½á°İLînh ÈÎÎñ", 6, x229024_g_JieBaiMission_IDX )		
			AddNumText( sceneId, x229024_g_ScriptId, "Ê¦Í½Lînh ÈÎÎñ", 6, x229024_g_ShiTuMission_IDX )		
		EndEvent(sceneId)
...
function x229024_OnEnumerate( sceneId, selfId, targetId )
	...
	if x229024_CheckHuoDongTime() == 1 or x229024_CheckSpecialTime() ==1 then --czf modify
		AddNumText( sceneId, x229024_g_ScriptId, "Ç§Ñ°", 6, x229024_g_QianXunMission_IDX )
	end
		
	AddNumText( sceneId, x229024_g_ScriptId, "¹ØÓÚÇ§Ñ°", 11, x229024_g_MissionInfo_IDX )
	...
	if canAdd > 0 then						
		...
		if bAdd >= 1 then				
			...
			BeginEvent(sceneId)
				...
				local strText = format("Ç§Ñ°ÈÎÎñµÚ1»·: ÄãÖ»ÓĞµ½%stoÕ ğµ #{_INFOAIM%d,%d,%d,%s} m¾i có th¬ sØ døngÑ°Â·Ö®ĞÄ", ScintInfo.scenename, ScintInfo.PosX, ScintInfo.PosY, ScintInfo.sceneId, ScintInfo.scenename)						
				AddText(sceneId,strText)
				AddText(sceneId,"#e00f000Nh¡c nhö: #e000000Ê¹ÓÃ#gfff0f0»¬ÊóÓÒ¼ü#g000000 ği¬m»÷ÈÎÎñ±³°üÖĞtoÕ ğµ #gfff0f0Ñ°Â·Ö®ĞÄ#g000000,¿ÉÒÔÏÔÊ¾C¥n ÕÒtoÕ ğµ ÈÎÎñµØ ği¬m.")											
			EndEvent( )					
			DispatchEventList(sceneId, selfId, targetId)
...
function x229024_OnUseItem( sceneId, selfId, bagIndex )
  ...
  if nRoundNum >= x229024_g_MaxRound then
  	local missitemid = GetItemTableIndexByIndex( sceneId, selfId, bagIndex )
		...
		x229024_NotifyTip( sceneId, selfId, "Íê³ÉÇé¿ö:ÒÑÌ½Ë÷ "..nRoundNum.."/"..x229024_g_MaxRound )
		...
		str = format("Ç§Ñ°ÈÎÎñµÚ%d»·: È¥#G%s#WÌ½Ë÷#G%s#{_INFOAIM%d,%d,%d,%s}#W,mµt ÀÀ½­ºşÃÀ¾°°É.µÖ´ïºóÇëÊ¹ÓÃ#YÑ°Â·Ö®ĞÄ#W.", nRoundNum, ScintInfo.scenename, ScintInfo.PosName, ScintInfo.PosX, ScintInfo.PosY, ScintInfo.sceneId, ScintInfo.scenename)	
