--MisDescBegin
x808038_g_ScriptId = 808038;
x808038_g_Position_X=170.5004
x808038_g_Position_Z=122.5593
x808038_g_SceneID=2
x808038_g_AccomplishNPC_Name="Cüng Thái Vân"
x808038_g_MissionId = 1130;
x808038_g_Name 					= "Cüng Thái Vân"
x808038_g_MissionKind			= 13 --´óÀí
x808038_g_MissionLevel		= 10000
x808038_g_IfMissionElite	= 0
x808038_g_IsMissionOkFail	= 0		--ÈÎÎñ²ÎÊýµÄµÚ0Î»
x808038_g_MissionName			= "Thu th§p chæ ký"
x808038_g_MissionInfo			= "Thu th§p xong 10 Häo Hæu Thi¬m Danh r°i, hãy mau ðªn Cung Thái Vân ð±i ph¥n thß·ng nào !"
x808038_g_MissionTarget		= "#{SJQM_8825_20}"
x808038_g_ContinueInfo		= "Chßa hoàn thành!"
x808038_g_MissionComplete	= "Làm r¤t t¯t!"
x808038_g_city 				 	= 1		 --ÈÎÎñ²ÎÊýµÄµÚ1Î»
x808038_g_Custom	= { {id="Ðã thu th§p ðü chæ ký",num=10} }
--MisDescEnd
