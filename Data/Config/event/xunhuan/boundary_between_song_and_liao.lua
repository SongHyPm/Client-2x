--MisDescBegin
x050100_g_ScriptId = 050100
x050100_g_MissionId = 1260					-- 1260 - 1269
x050100_g_activePointIndex = 6
x050100_g_Name = "Ti«n Hoành Vû"
x050100_g_MissionKind = 8
x050100_g_MissionLevel = 10000
x050100_g_MissionName = "Mµt tên cûng không th¬ thoát"
x050100_g_MissionInfo = "    "				-- ÈÎÎñÃèÊö
x050100_g_MissionTarget = "    Ti«n Hoành Vû · Tô Châu #{_INFOAIM62,162,1,Ti«n Hoành Vû } yêu c¥u các hÕ giªt chªt Dß Ğµc trong vòng 30 phút#r  #{FQSH_090206_01}"						-- ÈÎÎñÄ¿±ê
x050100_g_ContinueInfo = "    Tr§n chiªn vô cùng tr÷ng ğÕi, nh¤t ğ¸nh không ğßşc khinh xu¤t. Các hÕ ğã chu¦n b¸ ğªn sao huy®t b÷n cß¾p chßa ?"	-- Î´Íê³ÉÈÎÎñµÄnpc¶Ô»°
x050100_g_SubmitInfo = "    Nhi®m vø làm thª nào r°i?"										-- Ìá½»Ê±µÄ´ğ¸´
x050100_g_MissionComplete = "    T¯t quá, t× l®nh bài này, chúng ta s¨ tìm th¤y doanh trÕi cüa b÷n phï. Hãy ğªn tiêu cøc tìm Hoa Kiªm Vû#{_INFOAIM251,109,1,Hoa Kiªm Vû}, ông ta s¨ k¬ cho các hÕ nghe lai l¸ch cüa chúng."	--Íê³ÉÈÎÎñnpcËµµÄ»°
x050100_g_IsMissionOkFail = 0				-- 0 ºÅ£ºµ±Ç°ÈÎÎñÊÇ·ñÍê³É(0Î´Íê³É£»1Íê³É£»2Ê§°Ü)
x050100_g_DemandKill = { { id = 4060, num = 50 }, { id = 4070, num = 10 }, { id = 4080, num = 1 }, { id = 4090, num = 1 }, { id = 4100, num = 1 } }	-- 1 ~ 5 ºÅ£¬¹ÖÎïĞÅÏ¢
x050100_g_Param_sceneid = 6					-- 6 ºÅ£ºµ±Ç°¸±±¾ÈÎÎñµÄ³¡¾°ºÅ
--MisDescEnd
