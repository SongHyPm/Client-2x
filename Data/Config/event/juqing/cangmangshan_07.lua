--MisDescBegin
x200036_g_ScriptId = 200036
x200036_g_CopySceneName="Thß½ng Mang S½n "
x200036_g_MissionId = 31
x200036_g_PreMissionId = 30
x200036_g_Name = "Tiêu Phong"
x200036_g_IfMissionElite = 1
x200036_g_MissionLevel = 50
x200036_g_MissionKind = 52
x200036_g_MissionName="Kim Qua Ðãng Kh¤u Ngao Binh"
x200036_g_MissionInfo="#{Mis_juqing_0031}"
x200036_g_MissionTarget="#{Mis_juqing_Tar_0031}"	--ÈÎÎñÄ¿±ê
x200036_g_MissionComplete="    $N, l¥n này thì quân tình cüa phän quân ðã löng l¨o r°i, không c¥n vµi, chúng ta tiªn t×ng bß¾c vào doanh, tri®t ð¬ giäi tr× hªt b÷n phän loÕn. "	--Íê³ÉÈÎÎñnpcËµ»°µÄ»°
x200036_g_MissionContinue="  S· Vß½ng ðã b¸ giªt, Hoàng Thái Thúc cûng ðã r½i vào tay chúng ta"
x200036_g_MoneyBonus=36000
x200036_g_exp=40800
x200036_g_RadioItemBonus={{id=10423012 ,num=1},{id=10423013,num=1},{id=10423014,num=1}}
x200036_g_Custom	= { {id="Ðã giªt chªt S· Vß½ng",num=1},{id="Ðã vào ngân kh¯ riêng cüa Hoàng Thái Thúc",num=1} }
x200036_g_IsMissionOkFail = 0
--MisDescEnd
