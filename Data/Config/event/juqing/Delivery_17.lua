--MisDescBegin
x200087_g_ScriptId = 200087
x200087_g_MissionId = 17
x200087_g_PreMissionId = 16
x200087_g_Position_X=63
x200087_g_Position_Z=36
x200087_g_SceneID=2
x200087_g_AccomplishNPC_Name="Ðoàn Chính Thu¥n"
x200087_g_Name	="Ðoàn Chính Thu¥n"
x200087_g_MissionKind = 47
x200087_g_MissionLevel = 40
x200087_g_IfMissionElite = 0
x200087_g_MissionName="Qu¥n long vô thü"
x200087_g_MissionInfo="#{Mis_juqing_0017}"
x200087_g_MissionTarget="#{Mis_juqing_Tar_0017}"
x200087_g_MissionComplete="  Ha ha, thiªu hi®p lâu l¡m không g£p, vçn mÕnh giöi chÑ? T¾i ÐÕi Lý ta có chuy®n gì?"
x200087_g_MoneyBonus=5400
x200087_g_exp=5400
x200087_g_Custom	= { {id="Ðã tìm ðßþc Ðoàn Chính Thu¥n",num=1} }
x200087_g_IsMissionOkFail = 0
--MisDescEnd
