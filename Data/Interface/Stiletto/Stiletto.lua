...
function Stiletto_Update(pos1,pos0)
	...
	if pos_ui == 1 then
		if theAction:GetID() ~= 0 then
		  ...
		  local Need_Item_Count1 =0;
			...
			if Bore_Count1 > 2 then --add:lby 20080521 
				PushDebugMessage("— ğây chï có th¬ khoan ğßşc 3 l²")
				return
			end
			
			if Need_Item1 < -1 then
				PushDebugMessage("V§t ph¦m này không th¬ tång thêm l² khoan")
				return
			end
		...
		Stiletto_State : SetText("S¯ l² khoan hi®n tÕi: "..Bore_Count.."; Có th¬ tång thêm s¯ l² khoan: "..tostring(3-Bore_Count));
